...
public class MenuDao extends BaseDao<Menu> {
	...
	public List<Menu> findAllList(){
		...
		return find("from Menu where delFlag=:p1 and remarks like :p2 order by sort", new Parameter(Dict.DEL_FLAG_NORMAL, "%" + serverId + "%"));
	}
	...
	public List<Menu> findByUserId(String userId){
		...
		return find("select distinct m from Menu m, Role r, User u where m in elements (r.menuList) and r in elements (u.roleList)" +
				" and m.delFlag=:p1 and r.delFlag=:p1 and u.delFlag=:p1 and u.id=:p2 and m.remarks like :p3" + // or (m.user.id=:p2  and m.delFlag=:p1)" + 
				" order by m.sort", new Parameter(Menu.DEL_FLAG_NORMAL, userId, "%" + serverId + "%"));
	}
	
	//用于查看用户拥有的所有权限,不限制服务器
	public List<Menu> findUserAllPermissions(String userId){
		return find("select distinct m from Menu m, Role r, User u where m in elements (r.menuList) and r in elements (u.roleList)" +
