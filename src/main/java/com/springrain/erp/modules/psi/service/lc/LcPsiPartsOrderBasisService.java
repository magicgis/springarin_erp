...
public class LcPsiPartsOrderBasisService extends BaseService {
	
	@Autowired
	private 	LcPsiPartsOrderBasisDao 		psiPartsOrderBasisDao;
	@Autowired
	...
	@Autowired
	private 	LcPsiPartsOrderDao 			partsOrderDao;
	@Autowired
	...
	public LcPsiPartsOrderBasis get(Integer id) {
		return psiPartsOrderBasisDao.get(id);
	}  
	
	public Page<LcPsiPartsOrderBasis> find(Page<LcPsiPartsOrderBasis> page, LcPsiPartsOrderBasis psiPartsOrderBasis) {
	...
	public String save(LcPsiPartsOrderBasisTotal psiPartsOrderBasisTotal) throws Exception {
		Map<Integer,List<LcPsiPartsOrderBasis>> supplierMap = Maps.newHashMap();
		Integer orderId=0;
		Set<Integer> partsIdSet=Sets.newHashSet();
		for(LcPsiPartsOrderBasis orderBasis:psiPartsOrderBasisTotal.getItems()){
			...
			List<LcPsiPartsOrderBasis>  orderBasisList =Lists.newArrayList();
			...
			//更新库存信息
			LcPsiPartsInventory partsInventory=this.psiPartsInventoryService.getPsiPartsInventorys(partsId);
		...
		private String exeOrderInfo(Map<Integer,List<LcPsiPartsOrderBasis>> supplierMap,Set<Integer> partsIdSet,Date purchaseDate) throws Exception{
			//查出配件信息map
			Map<Integer,PsiParts> partsMap=psiPartsService.getPartsByIds(partsIdSet);
			Integer purchaseOrderId=0;
			String  purchaseOrderNo ="";
			StringBuffer sb = new StringBuffer("");
			for(Map.Entry<Integer,List<LcPsiPartsOrderBasis>> entry:supplierMap.entrySet()){
				PsiSupplier    supplier =null;
				LcPsiPartsOrder  partsOrder = new LcPsiPartsOrder();
				List<LcPsiPartsOrderItem> items =Lists.newArrayList();
				String supplierName="";
				Float totalAmount = 0f;
				for(LcPsiPartsOrderBasis basis:entry.getValue()){
					...
					LcPsiPartsOrderItem partsOrderItem = new LcPsiPartsOrderItem();
...
//				this.sendEmailToSupplier(partsOrder);
	...
	public boolean isExistPurchaseOrder(Integer orderId){
		String sql ="SELECT id FROM lc_psi_parts_order_basis AS a WHERE a.`purchase_order_id`=:p1 AND a.`cancel_sta`='0'  LIMIT 1";
	...
	public List<LcPsiPartsOrderBasis> findByPurchaseOrdeIdAndPartsId(Integer purchaseOrderId,String cancelSta) {
		DetachedCriteria dc = psiPartsOrderBasisDao.createDetachedCriteria();
	...
	public List<LcPsiPartsOrderBasis> findByLcPurchaseOrdeId(Integer purchaseOrderId,String cancelSta) {
		DetachedCriteria dc = psiPartsOrderBasisDao.createDetachedCriteria();
		dc.add(Restrictions.eq("purchaseOrderId", purchaseOrderId));
		dc.add(Restrictions.eq("cancelSta", cancelSta));
		return  psiPartsOrderBasisDao.find(dc);
	 ...
	 * 跟供应商发送邮件
	 */
	public void sendEmailToSupplier(LcPsiPartsOrder LcPsiPartsOrder) throws Exception{
		LcPsiPartsOrder.setSendEamil("1");
		SendEmail  sendEmail = new SendEmail();
		//往采购供应商发信 获取供应商模板
		PsiSupplier supplier = psiSupplierService.get(LcPsiPartsOrder.getSupplier().getId());
		LcPsiPartsOrder.setSupplier(supplier);
		String orderNo=LcPsiPartsOrder.getPartsOrderNo();
		...
		PdfUtil.genPartsOrderPdf(pdfFile, LcPsiPartsOrder);
