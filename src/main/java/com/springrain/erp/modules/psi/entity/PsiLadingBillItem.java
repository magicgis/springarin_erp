 ...
 */
@Entity
@Table(name = "psi_lading_bill_item")
@Cache(usage = CacheConcurrencyStrategy.NONE)
public class PsiLadingBillItem implements Serializable{
	...
	private 	PsiLadingBill		 ladingBill;         // 提单主
	private 	PurchaseOrderItem    purchaseOrderItem;  // 采购订单项     
	...
	private    Integer       timelyQuantity;             //暂存及时库存
	
	public PsiLadingBillItem() {
		super();
	}

	public PsiLadingBillItem(Integer id){
		this();
		this.id = id;
	}
	...
	public PsiLadingBill getLadingBill() {
		return ladingBill;
	}

	public void setLadingBill(PsiLadingBill ladingBill) {
...
//			totalAmount=Math.round(this.itemPrice*this.quantityLading*(100-purchaseOrderItem.getPurchaseOrder().getDeposit()))*0.01f;
//		}
//		return totalAmount;
//	}

	public PsiLadingBillItem(PsiLadingBillItem item) {
		super();
	...
	public Float  getVolume(){
		if(this.purchaseOrderItem.getProduct().getPackQuantity()>0){
			return this.purchaseOrderItem.getProduct().getBoxVolume().floatValue()*(this.quantityLading/(float)this.purchaseOrderItem.getProduct().getPackQuantity());
		}else{
			return 0f;
		}	
	}
	...
	public Float  getWeight(){
		if(this.purchaseOrderItem.getProduct().getPackQuantity()>0){
			return this.purchaseOrderItem.getProduct().getGw().floatValue()*(this.quantityLading/(float)this.purchaseOrderItem.getProduct().getPackQuantity());
		}else{
			return 0f;
		}	
	}
	...
	public BigDecimal getNoDepositTotalAmount(){
		return this.itemPrice.multiply(new BigDecimal(quantityLading));
	}
