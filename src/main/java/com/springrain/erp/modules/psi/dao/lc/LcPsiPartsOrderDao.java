 ...
 */
@Repository
public class LcPsiPartsOrderDao extends BaseDao<LcPsiPartsOrder> {
	
	public void  updateOrderSta(Integer orderId,String sta){
		Parameter parameter =new Parameter(orderId,sta);
		this.updateBySql("update lc_psi_parts_order set order_sta=:p2 where id = :p1 ", parameter);
	}
	...
	public void  updateOrderStaAndFinishedDate(Integer orderId,String sta,Date finishedDate){
		...
		this.updateBySql("update lc_psi_parts_order set order_sta=:p2 , receive_finished_date=:p3 where id = :p1 ", parameter);
	}
	...
	public void  updateCancelStaByOrderID(Integer productOrderId,String sta){
		...
		this.updateBySql("update lc_psi_parts_order set order_sta=:p2 where purchase_order_id = :p1 ", parameter);
	}
	
	public void updateOrderStaByPurchaseOrderId(Integer purchaseOrderId,String sta){
		String updateSql="";
		if("5".equals(sta)){
			updateSql="UPDATE lc_psi_parts_order AS a SET a.order_sta= CASE WHEN a.`payment_sta`='2' THEN '7' ELSE :p2 END WHERE a.purchase_order_id=:p1";
		}else{
			updateSql="UPDATE lc_psi_parts_order AS a SET a.order_sta= :p2  WHERE a.purchase_order_id=:p1";
		}
	...
	public  void  updatePrePaymentAmount(Integer partsOrderId,Float preAmount,String flag){
		...
		if("cancel".equals(flag)){
			sql ="UPDATE lc_psi_parts_order AS a SET a.`pre_payment_amount`=(a.`pre_payment_amount`-:p2) WHERE a.`id`=:p1 ";
		}else{
			sql ="UPDATE lc_psi_parts_order AS a SET a.`pre_payment_amount`=(a.`pre_payment_amount`+:p2) WHERE a.`id`=:p1 ";
		}
