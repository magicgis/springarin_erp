...
import com.springrain.erp.modules.psi.dao.lc.LcPsiPartsDeliveryDao;
...
import com.springrain.erp.modules.psi.dao.lc.LcPsiPartsPaymentDao;
import com.springrain.erp.modules.psi.dao.lc.LcPsiPartsPaymentItemDao;
...
public class LcPsiPartsPaymentService extends BaseService {
	@Autowired
	private LcPsiPartsPaymentDao   		LcPsiPartsPaymentDao;
	@Autowired
	private LcPsiPartsOrderDao	 		psiPartsOrderDao;
	@Autowired
	private LcPsiPartsDeliveryDao	 	psiPartsDeliveryDao;
	@Autowired
	...
	@Autowired
	private LcPsiPartsPaymentItemDao 	psiPartsPaymentItemDao;
	@Autowired
	private LcPsiPartsOrderItemDao      psiPartsOrderItemDao;
	       
	private static String filePath;
	
	public LcPsiPartsPayment get(Integer id) {
		return LcPsiPartsPaymentDao.get(id);
	}
	
	public Page<LcPsiPartsPayment> find(Page<LcPsiPartsPayment> page, LcPsiPartsPayment psiPartsPayment) {
		DetachedCriteria dc = LcPsiPartsPaymentDao.createDetachedCriteria();
		...
		return LcPsiPartsPaymentDao.find(page, dc);
	}
	
	public List<LcPsiPartsPayment> find(LcPsiPartsPayment psiPartsPayment) {    
		DetachedCriteria dc = LcPsiPartsPaymentDao.createDetachedCriteria();
		if(psiPartsPayment.getCreateDate()!=null){
			dc.add(Restrictions.ge("createDate",psiPartsPayment.getCreateDate()));
		}
		...
		dc.addOrder(Order.desc("id"));
		return LcPsiPartsPaymentDao.find(dc);
	}
	
	@Transactional(readOnly = false)
	public void addSave(LcPsiPartsPayment psiPartsPayment) throws Exception {
		...
		if(psiPartsPayment.getSupplier()!=null&&psiPartsPayment.getSupplier().getId()!=null){
			...
			String paymentNo=this.genSequenceDao.genSequence(supplier.getNikename()+"_LC_PJFK",2);
			...
			for(LcPsiPartsPaymentItem item : psiPartsPayment.getItems()) {
				...
				if(psiPartsPayment.getPaymentSta().equals("1")){
					if("0".equals(item.getPaymentType())){
						...
						LcPsiPartsDelivery bill=item.getLadingBill();
		...
		LcPsiPartsPaymentDao.save(psiPartsPayment);
	}
	
	
	@Transactional(readOnly = false)
	public void cancel(LcPsiPartsPayment psiPartsPayment) {
//		//把原来预交货的数量改回来
//		for(LcPsiPartsPaymentItem item :psiPartsPayment.getItems()){
...
//		}
		//追回提单预付款项        订单预付款项
		for(LcPsiPartsPaymentItem payItem:psiPartsPayment.getItems()){
			LcPsiPartsDelivery bill =payItem.getLadingBill();
			if(bill!=null){
				//提单不为空说明是尾款       否则为定金
				Float preAmonut = bill.getTotalPaymentPreAmount();
				LcPsiPartsDelivery oldLadingBill=this.psiPartsDeliveryDao.get(bill.getId());
				...
				LcPsiPartsOrder order=payItem.getOrder();
				//定金只能付一次，直接清空预付定金即可
		...
		this.LcPsiPartsPaymentDao.save(psiPartsPayment);
	}
	
	
	@Transactional(readOnly = false)
	public void sureSave(MultipartFile memoFile,LcPsiPartsPayment psiPartsPayment) {
			...
			for(LcPsiPartsPaymentItem item:psiPartsPayment.getItems()){
				...
				if(item.getPaymentType()!=null&&!"".equals(item.getPaymentType())){
					if("0".equals(item.getPaymentType())){
						...
						LcPsiPartsOrder order = item.getOrder();
						...
						if(Float.floatToIntBits(bill.getTotalAmount())==Float.floatToIntBits(bill.getTotalPaymentAmount())){
							for(LcPsiPartsDeliveryItem billItem: bill.getItems()){
								LcPsiPartsOrderItem orderItem =billItem.getPartsOrderItem();
								LcPsiPartsOrder     order     = orderItem.getPartsOrder();
...
//								}
			...
			for (Map.Entry<Integer, List<String>> entry : orderMap.entrySet()) {
				...
				for (LcPsiPartsOrderItem item : this.psiPartsOrderDao.get(orderId).getItems()) {
				...
				if(isFinal){
					LcPsiPartsOrder order = this.psiPartsOrderDao.get(orderId);
	...
	@Transactional(readOnly = false)
	public boolean editSave(LcPsiPartsPayment partsPayment) {
		...
		if(!rs){
			for(LcPsiPartsPaymentItem item : partsPayment.getItems()) {
		...
		Collection<String> setNewIds = Collections2.transform(partsPayment.getItems(), new Function<LcPsiPartsPaymentItem, String>() {
			public String apply(LcPsiPartsPaymentItem input) {
		...
		if(delItemSet.size()>0){
			for(LcPsiPartsPaymentItem item:this.getPartsPaymentItems(delItemSet)){
				item.setDelFlag("1");
				item.setPsiPartsPayment(partsPayment);
		...
		this.LcPsiPartsPaymentDao.getSession().merge(partsPayment);
		return rs;
	}
	
	
	
	private boolean replyEmailOrGetTotal(LcPsiPartsPayment partsPayment) throws Exception{
		partsPayment.setSupplier(psiSupplierService.get(partsPayment.getSupplier().getId()));
		Float totalAmount=0f;
		for(LcPsiPartsPaymentItem item : partsPayment.getItems()) {
			totalAmount+=item.getPaymentAmount();
			if("0".equals(item.getPaymentType())){
				LcPsiPartsOrder order = this.psiPartsOrderDao.get(item.getUnknowId());
				item.setOrder(order);
				if(partsPayment.getPaymentSta().equals("1")){
					order.setDepositPreAmount(item.getPaymentAmount());
				}
			}else if("1".equals(item.getPaymentType())){
				LcPsiPartsDelivery bill=this.psiPartsDeliveryDao.get(item.getUnknowId());
		...
		if(partsPayment.getPaymentSta().equals("1")){
			...
			partsPayment.setPayFlowNo(genSequenceDao.genSequenceByMonth2("PAR-", 4));
	...
	public void updateSta(Integer paymentId,String sta) {
		String sql ="UPDATE lc_psi_parts_payment AS a SET a.`payment_sta`=:p2 WHERE a.`id`=:p1";
		LcPsiPartsPaymentDao.updateBySql(sql,new Parameter(paymentId,sta));
	}
	
	
	@Transactional(readOnly = false)
	public void save(LcPsiPartsPayment psiPartsPayment) {
		LcPsiPartsPaymentDao.save(psiPartsPayment);
	}
	 ...
	 * 
	 */
	public  Map<String,LcPsiPartsOrder>   getUnPaymentDoneOrder(Integer supplierId,String currency){
		 Map<String,LcPsiPartsOrder> orderMap =Maps.newHashMap();
		String sql ="SELECT id,parts_order_no,(total_amount*deposit)/100,payment_amount,pre_payment_amount,deposit,currency_type,deposit_amount FROM lc_psi_parts_order  WHERE deposit>0 AND order_sta NOT IN('0','7','8') AND  deposit_pre_amount =0  AND  deposit_amount =0  AND supplier_id=:p1  ORDER BY id ";
		List<Object[]> list=null;
		if(StringUtils.isNotEmpty(currency)){
			sql ="SELECT id,parts_order_no,(total_amount*deposit)/100,payment_amount,pre_payment_amount,deposit,currency_type,deposit_amount FROM lc_psi_parts_order  WHERE deposit>0 AND order_sta NOT IN('0','7','8') AND  deposit_pre_amount =0  AND  deposit_amount =0  AND supplier_id=:p1 AND currency_type=:p2 ORDER BY id ";
		...
		for(Object[] obj :list){
			orderMap.put((String)obj[1],new LcPsiPartsOrder((Integer)obj[0], (String)obj[1], ((BigDecimal)obj[2]).floatValue(), ((BigDecimal)obj[3]).floatValue(), ((BigDecimal)obj[4]).floatValue(), (Integer)obj[5], (String)obj[6],((BigDecimal)obj[7]).floatValue()));
		}
		return orderMap;
	 ...
	 * 
	 */
	public  Map<String,LcPsiPartsDelivery>   getUnPaymentDoneLading(Integer supplierId,String currency){
		 Map<String,LcPsiPartsDelivery> ladingMap =Maps.newHashMap();
		String sql ="SELECT id,bill_no,(total_amount - total_payment_amount - total_payment_pre_amount ) as aaa,(total_payment_amount + total_payment_pre_amount) as bbb,currency_type FROM lc_psi_parts_delivery WHERE (total_amount-total_payment_amount - total_payment_pre_amount)>0  AND bill_sta ='1' AND supplier_id=:p1 ORDER BY id ";;
		List<Object[]> list=null;
		if(StringUtils.isNotEmpty(currency)){
		...
		for(Object[] obj :list){
			ladingMap.put((String)obj[1],new LcPsiPartsDelivery((Integer)obj[0], (String)obj[1], ((BigDecimal)obj[2]).floatValue(), ((BigDecimal)obj[3]).floatValue(), (String)obj[4]));
		}
		return ladingMap;
	}
	 ...
	 *根据itemId获取list信息 
	 */
	public List<LcPsiPartsPaymentItem> getPartsPaymentItems(Set<Integer> ids){
	...
	private void sendToSupplier(LcPsiPartsPayment partsPayment) throws Exception{
