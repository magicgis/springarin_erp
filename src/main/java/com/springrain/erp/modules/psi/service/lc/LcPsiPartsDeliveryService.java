...
public class LcPsiPartsDeliveryService extends BaseService {
	...
	public LcPsiPartsDelivery get(Integer id) {
		return psiPartsDeliveryDao.get(id);
	}
	
	public Page<LcPsiPartsDelivery> find(Page<LcPsiPartsDelivery> page, LcPsiPartsDelivery psiPartsDelivery) {
	...
	public void save(LcPsiPartsDelivery psiPartsDelivery) {
		psiPartsDeliveryDao.save(psiPartsDelivery);
	}
	
	
	@Transactional(readOnly = false)
	public void batchSave(LcPsiPartsDelivery psiPartsDelivery) {
		
		Integer total = 0;
		Float   totalAmount =0f ;
		
		for (Iterator<LcPsiPartsDeliveryItem> iterator = psiPartsDelivery.getItems().iterator(); iterator.hasNext();) {
			LcPsiPartsDeliveryItem item = (LcPsiPartsDeliveryItem) iterator.next();
			if(item.getQuantityLading()==null||item.getQuantityLading()==0){
				iterator.remove();
			}else{
				item.setPartsDelivery(psiPartsDelivery);
				LcPsiPartsOrderItem orderItem = psiPartsOrderItemDao.get(item.getPartsOrderItem().getId());
	...
	private void sendEmailToVendor(LcPsiPartsDelivery deliveryBill) throws Exception{
	...
	@Transactional( readOnly = false)
	public void sureSave(LcPsiPartsDelivery psiPartsDeliery,MultipartFile[] attchmentFiles) {
		//如果不为申请状态不能确认
		...
		for(LcPsiPartsDeliveryItem billItem: psiPartsDeliery.getItems()){
...
//				if(!orderMap.get(orderId).contains(productName)){
		...
		for (Map.Entry<Integer, String> entry: orderStatus.entrySet()) {
			...
			if("5".equals(entry.getValue())){
				//如果是已收货      获取收货时间  
				this.psiPartsOrderDao.updateOrderStaAndFinishedDate(id, entry.getValue(),new Date());
			}else{
				this.psiPartsOrderDao.updateOrderSta(id,entry.getValue());
			}
		}
	...
	public  void changePartsInventory(Map<Integer,Integer> partsMap,String billNo){
		for(Map.Entry<Integer,Integer> entry:partsMap.entrySet()){
			...
			LcPsiPartsInventory partsInventory= psiPartsInventoryService.getPsiPartsInventorys(partsId);
	...
	@Transactional(readOnly = false)
	public boolean cancelBill(LcPsiPartsDelivery psiPartsDelivery) {
		boolean flag=true;
		try{
		for(LcPsiPartsDeliveryItem item:psiPartsDelivery.getItems()){
			LcPsiPartsOrderItem orderItem=item.getPartsOrderItem();
			//获取预提单数量 
