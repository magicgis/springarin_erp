 ...
 */
@Repository
public class LcPurchaseAmountAdjustDao extends BaseDao<LcPurchaseAmountAdjust> {
	
	public void updateStaByPaymentId(Integer id,String sta ) {
		String sql ="UPDATE lc_psi_purchase_amount_adjust AS a SET a.`adjust_sta`=:p2 WHERE a.`payment_id`=:p1";
		this.updateBySql(sql, new Parameter(id,sta));
	}
	public void updateStaPaymentIdById(Integer id,String sta ,Integer paymentId) {
		String sql ="UPDATE lc_psi_purchase_amount_adjust AS a SET a.`adjust_sta`=:p2,a.`payment_id`=:p3 WHERE a.`id`=:p1";
