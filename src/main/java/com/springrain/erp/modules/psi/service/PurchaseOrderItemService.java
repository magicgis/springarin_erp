...
public class PurchaseOrderItemService extends BaseService {
	...
	public String findUnitPrice(Integer productId,String countryCode,String colorCode){
		String sql ="SELECT CASE WHEN b.`country_code`='jp' OR b.`country_code`='com' OR b.`country_code`='ca' THEN 1.1*b.item_price ELSE 1.2*b.item_price END ,a.`currency_type` FROM psi_purchase_order AS a,psi_purchase_order_item AS b WHERE a.`id`=b.`purchase_order_id` AND b.del_flag='0' AND a.`order_sta` IN ('2','3','4') AND b.product_id=:p1 AND b.country_code=:p2 AND b.color_code=:p3 ORDER BY b.id DESC  LIMIT 1";
	...
	public List<PurchaseOrderItem> getOrderItems(String items) {
		DetachedCriteria dc = purchaseOrderItemDao.createDetachedCriteria();
