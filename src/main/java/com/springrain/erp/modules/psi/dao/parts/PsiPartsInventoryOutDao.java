 ...
 */
@Repository
public class PsiPartsInventoryOutDao extends BaseDao<PsiPartsInventoryOut> {   
	//增加一条可收货信息
	public void insertProductCanDeliveryCheck(Integer purchaseOrderId,Integer productId,Integer quantity,String color){
		String sql="INSERT INTO psi_parts_delivery_check(purchase_order_id,product_id,can_lading_quantity,color) VALUES (:p1,:p2,:p3,:p4)";
	...
	public void addProductCanDeliveryQuantity(Integer purchaseOrderId,Integer productId,Integer quantity,String color){
		String sql="UPDATE psi_parts_delivery_check AS a SET a.`can_lading_quantity`=(a.`can_lading_quantity`+:p3) WHERE a.`purchase_order_id`=:p1 AND a.`product_id`=:p2 AND a.color=:p4";
		this.updateBySql(sql, new Parameter(purchaseOrderId,productId,quantity,color));
	}
	...
	public Integer getCanDeliveryQuantity(Integer purchaseOrderId,Integer productId,String color){
		String sql="SELECT a.`can_lading_quantity` FROM  psi_parts_delivery_check AS a WHERE a.`purchase_order_id`=:p1 AND a.`product_id`=:p2 AND a.color=:p3";
