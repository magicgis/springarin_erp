...
public class LcPsiPartsInventoryOutService extends BaseService {
	...
	public LcPsiPartsInventoryOut get(String id) {
		return psiPartsInventoryOutDao.get(id);
	}
	
	public Page<LcPsiPartsInventoryOut> find(Page<LcPsiPartsInventoryOut> page, LcPsiPartsInventoryOut psiPartsInventoryOut){
	...
	public void addSave(LcPsiPartsInventoryOut psiPartsInventoryOut) {
		this.exePartsInventoryOut(psiPartsInventoryOut);
		for(LcPsiPartsInventoryOutOrder order:psiPartsInventoryOut.getOrders()){
	 ...
	 * 
	 */
	private void exePartsInventoryOut(LcPsiPartsInventoryOut psiPartsInventoryOut){
		for(LcPsiPartsInventoryOutItem item:psiPartsInventoryOut.getItems()){
	...
	@Transactional(readOnly = false)
	public void save(LcPsiPartsInventoryOut psiPartsInventoryOut) {
		psiPartsInventoryOutDao.save(psiPartsInventoryOut);
	}
	...
	public Map<String,Integer> getCanLadingQuantity(Integer purchaseOrderId){
		...
		String sql="SELECT a.`product_id`,a.color,a.`can_lading_quantity` FROM  lc_psi_parts_delivery_check AS a WHERE a.`can_lading_quantity`>0 AND a.`purchase_order_id`=:p1 ";
	...
	public Map<Integer,Map<String,Integer>> getCanLadingQuantitys(Set<Integer> purchaseOrderIds){
		...
		String sql="SELECT a.`purchase_order_id`,a.`product_id`,a.color,a.`can_lading_quantity` FROM  lc_psi_parts_delivery_check AS a WHERE a.`can_lading_quantity`>0 AND a.`purchase_order_id` in :p1 ";
	...
	public List<LcPsiPartsInventoryOut> findByOrderId(Integer purchaseOrderId){
			Page<LcPsiPartsInventoryOut> page  = new Page<LcPsiPartsInventoryOut>();
