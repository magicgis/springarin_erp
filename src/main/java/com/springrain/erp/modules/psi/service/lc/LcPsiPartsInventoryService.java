...
public class LcPsiPartsInventoryService extends BaseService {
	@Autowired
	private LcPsiPartsInventoryDao psiPartsInventoryDao;
	@Autowired
	private LcPsiPartsInventoryLogDao psiPartsInventoryLogDao;
	
	public LcPsiPartsInventory get(Integer id) {
		return psiPartsInventoryDao.get(id);
	}
	
	public Page<LcPsiPartsInventory> find(Page<LcPsiPartsInventory> page, LcPsiPartsInventory psiPartsInventory) {
	...
	public void save(LcPsiPartsInventory psiPartsInventory) {
		psiPartsInventoryDao.save(psiPartsInventory);
	}
	
	@Transactional(readOnly = false)
	public void adjustSave(LcPsiPartsInventory psiPartsInventory) {
		String type=psiPartsInventory.getOperateType();
		for(LcPsiPartsInventoryLog log: psiPartsInventory.getPartsLogs()){
			Integer partsId = log.getPartsId();
			Integer quantity = log.getQuantity();
			log.setCreateDate(new Date());
			log.setCreateUser(UserUtils.getUser());
			LcPsiPartsInventory inventory =getPsiPartsInventorys(partsId);
			if("1".equals(type)){   
				//可用转换成冻结
	 ...
	 */
	public Map<Integer,LcPsiPartsInventory>  getPsiPartsInventorys(Set<Integer> partsId){
		DetachedCriteria dc = psiPartsInventoryDao.createDetachedCriteria();
		if(partsId!=null){
			dc.add(Restrictions.in("partsId", partsId));
		}
		List<LcPsiPartsInventory> partsInventorys =psiPartsInventoryDao.find(dc);
		 Map<Integer,LcPsiPartsInventory> partsMap =Maps.newHashMap();
		for(LcPsiPartsInventory inventory:partsInventorys){
	 ...
	 *根据partsId  查出该配件库存信息 
	 */
	public LcPsiPartsInventory  getPsiPartsInventorys(Integer partsId){
		DetachedCriteria dc = psiPartsInventoryDao.createDetachedCriteria();
	...
	public void updateInventoryByPartsId(Integer partsId,Integer quantity){
		String sql="UPDATE lc_psi_parts_inventory AS a SET a.`stock_frozen`=(a.`stock_frozen`-:p1) WHERE a.`parts_id`=:p2";
	...
	public Map<Integer,Integer> getPartsInventoryMap(Set<Integer> partsIdSet){
		...
		String sql ="SELECT a.`parts_id`,a.`stock_frozen` FROM lc_psi_parts_inventory AS a ";
