 ...
 */
@Entity
@Table(name = "psiPartsOrder")
public class PsiPartsOrder implements Serializable{   
	private static final long serialVersionUID = 6327815820579219156L;
	...
	private		 PsiPartsPaymentItem    payItem;         // 付款明细项
	
	private      List<PsiPartsOrderItem> items = Lists.newArrayList();
	public  PsiPartsOrder(){
		super();
	}
	
	public  PsiPartsOrder(Integer id){
		this();
		this.id=id;
	}
	...
	public List<PsiPartsOrderItem> getItems() {
		return items;
	}
	public void setItems(List<PsiPartsOrderItem> items) {
		this.items = items;
	}
	...
	public Integer getItemsQuantityReceived() {
		int rs = 0;
		for (PsiPartsOrderItem item : items) {
	...
	@NotFound(action = NotFoundAction.IGNORE)
	public PsiPartsPaymentItem getPayItem() {
		return payItem;
	}

	public void setPayItem(PsiPartsPaymentItem payItem) {
	...
	@Transient
	public Map<String,PsiPartsDelivery> getLadingsMap(){
		Map<String,PsiPartsDelivery> rs = Maps.newLinkedHashMap();
		for (PsiPartsOrderItem item : items) {
			for(PsiPartsDeliveryItem billItem:item.getDeliveryItemList()){
				String key= billItem.getPartsDelivery().getBillNo();
				PsiPartsDelivery temp = rs.get(key);
				if(temp==null){
					temp= new PsiPartsDelivery();
	...
	@Transient
	public Map<String,PsiPartsPayment> getPaymentsMap(){
		Map<String,PsiPartsPayment> rs = Maps.newLinkedHashMap();
		for (PsiPartsOrderItem item : items) {
			for(PsiPartsDeliveryItem billItem:item.getDeliveryItemList()){
				for(PsiPartsPaymentItem payItem:billItem.getPartsDelivery().getPayItems()){
					String key= payItem.getPsiPartsPayment().getPaymentNo();
					PsiPartsPayment temp = rs.get(key);
					if(temp==null){
	...
	public  PsiPartsOrder(Integer id,String partsOrderNo,Float totalAmount,Float paymentAmount,Float prePaymentAmount,Integer deposit,String currencyType,Float depositAmount){
		this();
		this.id=id;
		this.partsOrderNo=partsOrderNo;
		this.totalAmount=totalAmount;
