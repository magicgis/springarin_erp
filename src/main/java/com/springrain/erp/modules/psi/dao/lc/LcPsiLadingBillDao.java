 ...
 */
@Repository
public class LcPsiLadingBillDao extends BaseDao<LcPsiLadingBill> {
	
	public void deleteOrderItem(Integer itemId) {
		Parameter parameter =new Parameter(itemId);
		this.updateBySql("update lc_psi_lading_bill_item set del_flag='1' where id =:p1", parameter);
	}
	
	//查出提单未支付完的，已支付的金额
	public Float getFinalPayment(Integer supplierId){
		String getFinalSql="SELECT SUM( CASE WHEN ps.currency_type='CNY' THEN plb.total_payment_amount/"+AmazonProduct2Service.getRateConfig().get("USD/CNY")+"  ELSE plb.total_payment_amount END  ) FROM lc_psi_lading_bill AS plb,psi_supplier AS ps WHERE plb.supplier_id =ps.id AND plb.supplier_id=:p1 " +
