...
import com.springrain.erp.modules.psi.dao.PurchaseFinancialDao;
...
public class PurchaseFinancialService extends BaseService {

	@Autowired
	private PurchaseFinancialDao  purchaseFinancialDao;
	
	public List<PurchaseFinancialDto> findAll(){
		return purchaseFinancialDao.findAll();
	}
	
	
	@Transactional(readOnly = false)
	public void save(PurchaseFinancialDto purchaseFinancialDto) {
	...
	public Map<String,Float> getLadingFinancialReport(Float currentyRate,Set<String> keySet){
		...
		" CASE WHEN c.color_code!='' THEN CONCAT(c.product_name,'_',c.color_code) ELSE c.product_name	 END AS productName,a.supplier_id FROM psi_purchase_payment AS a ,"+
		" psi_purchase_payment_item AS b,psi_lading_bill_item AS c WHERE a.`id`=b.`payment_id` AND b.lading_item_bill_id=c.id AND a.`payment_sta`='2' AND b.`del_flag`='0' AND "+
		" b.`payment_type`='1'"+
	 ...
	 */
		public Map<String,Float> getPartsLadingFinancialReport(Float currentyRate,Set<String> keySet){
			...
			"CASE WHEN c.color_code!='' THEN CONCAT(c.product_name,'_',c.color_code) ELSE c.product_name	 END AS productName,a.supplier_id FROM psi_purchase_payment AS a ," +
			"psi_purchase_payment_item AS b,(SELECT a.id,a.supplier_id,b.`product_name`,b.`color_code`,SUM(b.`quantity_lading`*b.`item_price`)/a.`no_deposit_amount`  AS pcent FROM" +
			" psi_lading_bill AS a ,psi_lading_bill_item AS b WHERE a.`id`=b.`lading_bill_id` AND b.`del_flag`='0' AND a.`bill_sta`<>'2' GROUP BY  a.id,a.supplier_id," +
		...
		public Map<String,Float> getPartsOrderFinancialReport(Float currentyRate,Set<String> keySet){
			...
			String sql ="SELECT DATE_FORMAT(dd.create_date,'%Y%m'),dd.supplier_id,dd.productName,SUM(total) FROM  (SELECT a.`create_date`,TRUNCATE((CASE  WHEN a.currency_type ='CNY' THEN  b.`payment_amount`/:p1 ELSE  b.`payment_amount` END) * c.pcent *(a.`real_payment_amount`/a.`payment_amount_total`),2) AS total ," +
			"CASE WHEN c.color_code!='' THEN CONCAT(c.product_name,'_',c.color_code) ELSE c.product_name	 END AS productName,a.supplier_id FROM psi_purchase_payment AS a ," +
			"psi_purchase_payment_item AS b,(SELECT a.id,a.`supplier_id`,b.`product_name`,b.`color_code`,SUM(b.`quantity_ordered`*b.`item_price`)/a.`order_total` AS pcent FROM " +
			"psi_purchase_order AS a ,psi_purchase_order_item AS b WHERE a.`id`= b.`purchase_order_id` AND b.`del_flag`='0' AND a.order_sta<>'6' GROUP BY a.id,a.supplier_id," +
	...
	public Map<String,Float> getPurchaseReport(Float currentyRate,Set<String> keySet){
		...
		" FROM psi_purchase_order AS a ,psi_purchase_order_item AS b WHERE a.`id`= b.`purchase_order_id` AND b.`del_flag`='0' " +
		" AND a.order_sta <>'6'  GROUP BY DATE_FORMAT(a.create_date,'%Y%m'),a.supplier_id,b.product_name,b.color_code";
		List<Object[]> list = this.purchaseFinancialDao.findBySql(sql,new Parameter(currentyRate));
		if(list!=null&&list.size()>0){
			for(Object[] obj:list){
				if(obj[3]!=null){
	...
	public void createFinancicalReprotData(){
		//清空原有数据
		String sql="DELETE FROM psi_purchase_financial_report ";
		this.purchaseFinancialDao.updateBySql(sql,null);
		
		
		Float currentyRate = AmazonProduct2Service.getRateConfig().get("USD/CNY");
	...
	public Map<String,Set<Integer>>  getProductSupplierMap(){
		...
		String  sql= "SELECT a.`product_name_color`,a.`supplier_id` FROM psi_purchase_financial_report AS a  GROUP BY a.`supplier_id`,a.`product_name_color`";
	...
	public List<PurchaseFinancialDto> getFinancicalReport(PurchaseOrder order,String startMonthStr,String endMonthStr){
		String whereSql="";
		Parameter para = null;
		List<Object[]> list = null;
		List<PurchaseFinancialDto> rsList = Lists.newArrayList();
		...
		String sql="SELECT a.`month`,SUM(a.`order_amount`) as orderAmont,SUM(a.`pay_order_amount`) as payOrderAmount,SUM(a.`pay_lading_amount`) as payLadingAmount FROM psi_purchase_financial_report AS a WHERE a.month<='"+endMonthStr+"' AND a.month>='"+startMonthStr+"' "+whereSql+" GROUP BY a.month ORDER BY a.`month` DESC ";
		if(para!=null){
		...
		for(int i=0;i<list.size();i++){
			...
			PurchaseFinancialDto dto = new PurchaseFinancialDto(obj[0].toString(), null, null, Float.parseFloat(obj[1].toString()), Float.parseFloat(obj[2].toString()), Float.parseFloat(obj[3].toString()), null);
			rsList.add(dto);
		}
	...
	public List<PurchaseFinancialDto> getFinancicalReport(LcPurchaseOrder order,String startMonthStr,String endMonthStr){
		String whereSql="";
		Parameter para = null;
		List<Object[]> list = null;
		List<PurchaseFinancialDto> rsList = Lists.newArrayList();
	...
	public Map<Integer,Map<String,Float>> getFinancicalReportExport(PurchaseOrder order,String startMonthStr,String endMonthStr){
		...
		String sql="SELECT supplier_id,a.`month`,SUM(a.`order_amount`) as orderAmont FROM psi_purchase_financial_report AS a WHERE a.month<='"+endMonthStr+"' AND a.month>='"+startMonthStr+"' "+whereSql+" GROUP BY a.month,supplier_id DESC ";
		if(para!=null){
			list = this.purchaseFinancialDao.findBySql(sql,para);
		}else{
			list = this.purchaseFinancialDao.findBySql(sql);
