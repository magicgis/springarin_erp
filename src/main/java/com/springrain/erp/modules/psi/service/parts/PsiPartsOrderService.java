...
public class PsiPartsOrderService extends BaseService {
	...
	@Autowired 
	private     PsiPartsOrderBasisDao       partsOrderBasisDao;
	@Autowired
	...
	public PsiPartsOrder get(Integer orderId){
		return this.psiPartsOrderDao.get(orderId);
	}  
	
	public Page<PsiPartsOrder> find(Page<PsiPartsOrder> page, PsiPartsOrder psiPartsOrder) {
	...
	public void addSave(PsiPartsOrder psiPartsOrder, String nikeName) throws Exception {
		if(psiPartsOrder.getId()==null){
			//生成配件号
			String purchaseOrderNo=this.genSequenceDao.genSequence("ZJ_"+nikeName,2);
			Integer quantity=0;
	...
	@Transactional(readOnly = false)
	public void editSave(PsiPartsOrder psiPartsOrder) {
		if(psiPartsOrder.getId()!=null){
			...
			if(delItemSet.size()>0){
				for(PsiPartsOrderItem item:this.getPartsOrderItems(delItemSet)){
					item.setDelFlag("1");
					item.setPartsOrder(psiPartsOrder);
	...
	@Transactional(readOnly = false)
	public void sureSave(PsiPartsOrder psiPartsOrder) {
		if(psiPartsOrder.getId()!=null){
			Float   totalAmount =0f;
			for(PsiPartsOrderItem item:psiPartsOrder.getItems()){
				item.setPartsOrder(psiPartsOrder);
	...
	public void changePoInventory(PsiPartsOrder psiPartsOrder) {
		for(PsiPartsOrderItem item:psiPartsOrder.getItems()){
			PsiPartsInventory partsInventory= psiPartsInventoryService.getPsiPartsInventorys(item.getPsiParts().getId());
	...
	//更新po库存
	@Transactional(readOnly = false)
	public void productReceive(PsiPartsOrder psiPartsOrder) {
		for(PsiPartsOrderItem item:psiPartsOrder.getItems()){
			PsiPartsInventory partsInventory= psiPartsInventoryService.getPsiPartsInventorys(item.getPsiParts().getId());
	...
	@Transactional(readOnly = false)
	public void save(PsiPartsOrder psiPartsOrder) {
		psiPartsOrderDao.save(psiPartsOrder);
	}
	...
	@Transactional(readOnly = false)
	public void cancel(PsiPartsOrder psiPartsOrder) {
		if(StringUtils.isEmpty(psiPartsOrder.getPurchaseOrderNo())&&psiPartsOrder.getPurchaseOrderId()==null){
			...
			if("1".equals(psiPartsOrder.getOrderSta())){
				for (PsiPartsOrderItem item : psiPartsOrder.getItems()) {
					...
					PsiPartsInventory inventory=psiPartsInventoryDao.get(partsId);
			...
			List<PsiPartsOrderBasis> basisList=this.psiPartsOrderBasisService.findByPurchaseOrdeIdAndPartsId(purchaseOrderId,"0");
			if(basisList!=null&&basisList.size()>0){
				for(PsiPartsOrderBasis basis:basisList){
					Integer partsId=basis.getPartsId();
					String partsName = basis.getPartsName();
					PsiPartsInventory inventory=psiPartsInventoryService.getPsiPartsInventorys(partsId);
	...
	public void updateSendSta(Integer partsId,String sendSta){
		String sql ="UPDATE psi_parts_order AS a SET a.`send_eamil`=:p2 WHERE a.`id`=:p1";
	 ...
	 *根据itemId获取list信息 
	 */
	public List<PsiPartsOrderItem> getPartsOrderItems(Set<Integer> ids){
	...
	public boolean  getUnReceivedPartsOrder(Set<Integer> orderIdSet){
		String sql="SELECT b.purchase_order_id FROM psi_parts_order_item AS a,psi_parts_order AS b WHERE  a.parts_order_id=b.id AND a.del_flag='0' AND b.`order_sta`<>'8' AND b.purchase_order_id IN :p1 GROUP BY b.purchase_order_id LIMIT 1";
	...
	public Map<String,Integer>  getUnReceivedQuantity(Set<Integer> orderIdSet){
		...
		String sql="SELECT b.purchase_order_id,a.product_id, a.quantity_ordered-a.quantity_received FROM psi_parts_order_item AS a,psi_parts_order AS b WHERE  a.parts_order_id=b.id AND a.del_flag='0' AND b.`order_sta`<>'8' AND b.purchase_order_id IN :p1 GROUP BY b.purchase_order_id,a.product_id";
		List<Object[]> lists=this.psiPartsOrderDao.findBySql(sql, new Parameter(orderIdSet));
		if(lists.size()>0){
	...
	public  void  updatePaymentAmountAndSta(Integer partsOrderId,Float preAmount){
		String sql ="UPDATE psi_parts_order AS a SET a.`payment_amount`=(a.`payment_amount`+:p2), a.`pre_payment_amount`=(a.`pre_payment_amount`-:p2),CASE WHEN a.`total_amount`=(a.`payment_amount`+10080.00) THEN a.`payment_sta`='2'  ELSE a.`payment_sta`='1' END  WHERE a.`id`=:p1 ";
