...
public class LcHisPurchaseOrder implements Serializable{
	...
	private List<LcHisPurchaseOrderItem> items = Lists.newArrayList();
	
	
	public LcHisPurchaseOrder() {
		super();
	}

	public LcHisPurchaseOrder(Integer id){
		this();
		this.id = id;
	}
...
//		BigDecimal rs = 0f;
//		for (LcHisPurchaseOrderItem item : items) {
//			rs+=item.getPaymentAmount();
//		}
	...
	public List<LcHisPurchaseOrderItem> getItems() {
		return items;
	}
	
	public void setItems(List<LcHisPurchaseOrderItem> items) {
		this.items = items;
		if(items.size()>0){
			for(LcHisPurchaseOrderItem item:items){
	...
	@Transient
	public Map<String,Map<String,LcHisPurchaseOrder>> getOrderItemMap(){
		Map<String,Map<String,LcHisPurchaseOrder>>  rs = Maps.newHashMap();
		for (LcHisPurchaseOrderItem item : items) {
			String productName = item.getProduct().getName();
			Map<String,LcHisPurchaseOrder> tempMap = rs.get(productName);
			if(tempMap==null){
			...
			LcHisPurchaseOrder order = tempMap.get(color);
			if(order == null){
				order = new LcHisPurchaseOrder();
	...
	@Transient
	public Map<String,LcHisPurchaseOrder> getOrderItemsMap(){
		Map<String,LcHisPurchaseOrder>  rs = Maps.newLinkedHashMap();
		for (LcHisPurchaseOrderItem item : items) {
			String name = item.getProductName();
			LcHisPurchaseOrder temp=rs.get(name);
			if(temp==null){
				temp = new LcHisPurchaseOrder();
				rs.put(name, temp);
			}
	...
	@Transient
	public List<LcHisPurchaseOrder> getTempOrders(){
		Map<String,LcHisPurchaseOrder> map = Maps.newLinkedHashMap();
		List<LcHisPurchaseOrder> orders = Lists.newArrayList();
		for (LcHisPurchaseOrderItem item : items) {
			LcHisPurchaseOrder temp=map.get(item.getProductName());
			if(temp==null){
				temp=new LcHisPurchaseOrder();
	...
	public LcHisPurchaseOrder(LcPurchaseOrder order){
		...
		if(order.getItems()!=null && order.getItems().size()>0){
			List<LcHisPurchaseOrderItem> rs = Lists.newArrayList();
			for (LcPurchaseOrderItem item : order.getItems()) {
				rs.add(new LcHisPurchaseOrderItem(item, this));
			}
			this.items = rs;
		}
	}
	...
	@Transient
	public List<LcHisPurchaseOrder> getTempOrdersByDeliveryDate(){
		Map<String,LcHisPurchaseOrder> map = Maps.newLinkedHashMap();
		List<LcHisPurchaseOrder> orders = Lists.newArrayList();
		for (LcHisPurchaseOrderItem item : items) {
			LcHisPurchaseOrder temp=map.get(item.getProductName()+","+item.getActualDeliveryDate());
			if(temp==null){
				temp=new LcHisPurchaseOrder();
				temp.setSupplier(supplier);
				temp.setOrderSta(orderSta);
