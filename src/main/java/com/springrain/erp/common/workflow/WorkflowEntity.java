package com.springrain.erp.common.workflow;
		...
		if(historicVariableInstances!=null && historicVariableInstances.size()>0) {
			for(HistoricVariableInstance historicVariableInstance:historicVariableInstances) {
				if(null!=historicVariableInstance.getTaskId())
					map.put(historicVariableInstance.getTaskId(), historicVariableInstance.getValue());
			}
		}
		return map;
	}
	public Map<String, Object> getCommentMap(){
		...
		if(comments!=null && comments.size()>0) {
			for(Comment comment:comments) {
				map.put(comment.getTaskId(),((CommentEntity)comment).getMessage());
			}
		}
		return map;
	}
