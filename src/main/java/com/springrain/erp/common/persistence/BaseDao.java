...
//	        page.setCount(Long.valueOf(createSqlQuery(countSqlString, parameter).uniqueResult().toString()));
	 ...
	 * @return
	 */
	public Page<T> find2(Page<T> page, DetachedCriteria detachedCriteria) {
		// get count
		if (!page.isDisabled() && !page.isNotCount()){
			page.setCount(count2(detachedCriteria));
			if (page.getCount() < 1) {
				return page;
			}
		}
	...
	public long count2(DetachedCriteria detachedCriteria) {
		Criteria criteria = detachedCriteria.getExecutableCriteria(getSession());
		long totalCount = 0;
		try {
			// Get orders
