...
//        } 
        ...
        //对一个文件进行压缩处理
      String url = "e:/"; 
      String name  =  "Inateck MP1305.png";
      File baseDir = new File(url+"temp");
		if (!baseDir.isDirectory())
			baseDir.mkdirs();
      img_change(url,"temp",name);
      System.out.println("finish");
    }
     
 
    public static void img_change(String url,String temp,String name) 
    { 
            //Tosmallerpic(url,new File(url+name),temp,name,188,165,(float)0.7); 
           // Tosmallerpic(url,new File(url+name),temp,name,45,45,(float)0.7); 
            Tosmallerpic(url,new File(url+name),temp,name,116,100,(float)0.7); 
    } 
    ...
    * @param per 百分比 
    */ 
    private static void  Tosmallerpic(String f,File file,String p,String n,int w,int h,float per){ 
            Image src; 
            try { 
                src  =  javax.imageio.ImageIO.read(file); //构造Image对象 
 
               String img_midname  =  f+p+"/"+n; 
