...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/oa/buyDevice/list/task">待办列表</a></li>
	...
	<form:form id="searchForm" modelAttribute="buyDevice" action="${ctx}/oa/buyDevice/list/task" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<div>			
		<label>创建日期 ：</label><input style="width: 100px" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',onpicked:function(){$('#searchForm').attr('action','${ctx}/oa/buyDevice/list/task');$('#searchForm').submit();return true}});" readonly="readonly"  class="Wdate" type="text" name="createDateStart" value="<fmt:formatDate value="${buyDevice.createDateStart}" pattern="yyyy-MM-dd"/>" class="input-small" id="start"/>
				&nbsp;至&nbsp;<input style="width: 100px" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',onpicked:function(){$('#searchForm').attr('action','${ctx}/oa/buyDevice/list/task');$('#searchForm').submit();return true}});" readonly="readonly"  class="Wdate" type="text" name="createDateEnd" value="<fmt:formatDate value="${buyDevice.createDateEnd}" pattern="yyyy-MM-dd" />" id="end" class="input-small"/>
				&nbsp;&nbsp;&nbsp;&nbsp;
		</div>
	...
	<table style="width: 95%" id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr>
			...
			<th style="width: 220px">设备清单</th>
			<th style="width: 220px">设备采购理由</th>
		...
		<c:forEach items="${page.list}" var="buyDevice">
			<tr>
				...
				<td>
					<a class="encode" href="${ctx}/oa/buyDevice/detail?id=${buyDevice.id}&audit=1&state=${buyDevice.processStatus}">${buyDevice.processStatus}</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
