...
<body>
	...
	<form class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				${leave.applyDay}天${leave.applyHour}小时
			</div>
		</div>
		<c:if test="${leave.processStatus eq '已完成'}">
			<div class="control-group">
				<label class="control-label">实际开始时间：</label>
				<div class="controls">
					${leave.realityStartTime}
				</div>
			</div>
			<div class="control-group">
				<label class="control-label">实际结束时间：</label>
				<div class="controls">
					${leave.realityEndTime}
				</div>
			</div>
			<div class="control-group">
				<label class="control-label">实际请假天数：</label>
				<div class="controls">
					${leave.realityDay}天${leave.realityHour}小时
				</div>
			</div>
		</c:if>
		<div class="control-group">
				...
				<c:forEach items="${workflowEntity.historicTaskInstances}" var="historicTaskInstance">
					<c:if test="${not empty  historicTaskInstance.endTime}">
						<tr>
							...
							<td><fmt:formatDate value="${historicTaskInstance.endTime}" pattern="yyyy-MM-dd H:mm"/></td>
	...
	<c:if  test="${leave.audit}">
		<c:if test="${leave.processStatus eq '部门主管审批'}">
		...
		<c:if test="${leave.processStatus eq '调整申请'}">
			<form:form id="inputForm" modelAttribute="leave" action="${ctx}/oa/leave/modifyApply" method="post" class="form-horizontal">
				...
				<div class="control-group">
					...
					<div class="controls">
						<form:input path="applyDay" type="text" maxlength="3" class="required number"/>天
						<form:input path="applyHour" type="text" maxlength="3" class="required price" max="8"/>小时
					</div>
				</div>
				<div class="control-group">
					<label class="control-label">请假原因：</label>
		...
		<c:if test="${leave.processStatus eq '人事记录'}">
		...
		<c:if test="${leave.processStatus eq '总经理审批'}">
			<form:form id="inputForm" modelAttribute="leave" action="${ctx}/oa/leave/mgrAudit" method="post" class="form-horizontal">
				<form:hidden path="id"/>
				<form:hidden path="pass"/>
				<div class="control-group">
					<label class="control-label">审批备注：</label>
		...
		<c:if test="${leave.processStatus eq '销假'}">
			<form:form id="inputForm" modelAttribute="leave" action="${ctx}/oa/leave/reportBack" method="post" class="form-horizontal">
				...
				<div class="control-group">
					<label class="control-label">实际请假天数：</label>
					<div class="controls">
						<input id="realityDay" name="realityDay" value="${leave.applyDay}" type="text" maxlength="3" class="required number"/>天
						<input id="realityHour" name="realityHour" value="${leave.applyHour}" type="text" maxlength="3" class="required price" max="8"/>小时
					</div>
				</div>
				<div class="form-actions">
