...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/custom/sendEmail"><spring:message code="custom_email_sentList" /></a></li>
		<li><a href="${ctx}/custom/sendEmail/form"><spring:message code="custom_email_sentEmail"/></a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="sendEmail" action="${ctx}/custom/sendEmail" method="post" class="breadcrumb form-search">
		...
		<div style="height: 40px;line-height: 40px">
			...
			<label>SentState:
			<select name="sendFlag"  style="width: 222px;" id='sendFlag'>
					 <option value="2" ${'2' eq sendEmail.sendFlag?'selected':'' }>Undelivered message</option>
					 <option value="0" ${'0' eq sendEmail.sendFlag?'selected':'' }>Unsent</option>
					 <option value="1" ${'1' eq sendEmail.sendFlag?'selected':'' }>Sent</option>
			</select>
			&nbsp;&nbsp;
		...
		<c:forEach items="${page.list}" var="email">
			<tr>
				...
				<td>
					${email.type eq '0'?'发送':''}
					${email.type eq '1'?'回复':''}
					${email.type eq '2'?'采购订单':''}
					<c:if test="${not empty email.remark}">
