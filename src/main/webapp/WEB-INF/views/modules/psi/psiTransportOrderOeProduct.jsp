...
<head>
	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			if(!(top)){
				...
				if($("select#oceanModel").val()=='FCL'&&'${fn:length(psiTransportOrder.containerItems)>0}'){
				...
				$("#unitPrice").on("blur",function(){
					...
					$("input[name='tranAmount']").val(unitPrice*volume);
					//光标移动到tranAmount上
					$("input[name='tranAmount']").focus();
				});
				...
				$("#add-row").on("click",function(e){
		            ...
		            tr.append("<td><input type='text' maxlength='11' style='width: 80%'  name='lowerPrice' class='lowerPrice'/><input type='hidden' maxlength='11' style='width: 80%' name='productPrice' value='${item.productPrice}' class='productPrice'/></td>");
		            tr.append("<td><input type='text' maxlength='11' style='width: 80%'  name='importPrice' class='importPrice'/></td>");
		            ...
		            tr.append("<td><a href='#' class='remove-row'><span class='icon-minus'></span></a></td>");
					tbody.append(tr);
				...
				$("#add-row-container").on("click",function(e){
					e.preventDefault();
				...
				$("#splitOrder").on("click",function(e){
					top.$.jBox.confirm("确认要拆分运单吗?","<spring:message code="sys_label_tips_msg"/>",function(v,h,f){
						if(v=="ok") {
							 $("#inputForm").attr("action","${ctx}/psi/psiTransportOrder/splitEditSave");
							 $("#inputForm").submit();
							 $("#inputForm").attr("action","${ctx}/psi/psiTransportOrder/productEditSave");
						}
					},{buttonsFocus:1});
					...
					top.$.jBox.confirm("确认要创建FBA贴吗?<br/>FBA目的地: <select id='country'>"+   
						...
						if(v=="ok") {
							if(h.find("#country").val()){
							 ...
							 $("#inputForm").attr("action","${ctx}/psi/psiTransportOrder/splitGenFba");
							 $("#inputForm").submit();
							 $("#inputForm").attr("action","${ctx}/psi/psiTransportOrder/productEditSave");
						}
					},{buttonsFocus:1});
	...
	<body>
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="#">${empty psiTransportOrder.id ?'新建':'编辑'}${psiTransportOrder.model eq '1'?'海运':'铁路'}运单</a></li>
		</ul><br/>
		<form:form id="inputForm" modelAttribute="psiTransportOrder" action="${ctx}/psi/psiTransportOrder/productEditSave" method="post" class="form-horizontal">
			...
			<blockquote>
				...
				<div style="float:left;width:98%">
					<div class="control-group" style="float:left;width:25%;height:25px">
						<label class="control-label" style="width:100px">${psiTransportOrder.model eq '1'?'海运':'铁路'}模式:</label>
						<div class="controls" style="margin-left:120px" >
							<select name="oceanModel" style="width: 100%" id="oceanModel"  disabled>
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
				<label class="control-label" style="width:100px">起运${psiTransportOrder.model eq '1'?'港口':'火车站'}:</label>
				<div class="controls" style="margin-left:120px" >
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
				<label class="control-label" style="width:100px">目的${psiTransportOrder.model eq '1'?'港口':'火车站'}:</label>
				<div class="controls" style="margin-left:120px" >
		...
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:25%;height:25px">
					<label class="control-label" style="width:100px">${psiTransportOrder.model eq '1'?'船舶':'火车'}公司:</label>
					<div class="controls" style="margin-left:120px" >
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
					...
					<div class="controls" style="margin-left:120px" >
						<input name="etdDate"  type="text" readonly style="width:95%" value="<fmt:formatDate value="${psiTransportOrder.etdDate}" pattern="yyyy-MM-dd" />" />
					</div>
			</div>
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
					...
					<div class="controls" style="margin-left:120px" >
						<input name="preEtaDate" type="text" readonly style="width:95%" value="<fmt:formatDate value="${psiTransportOrder.preEtaDate}" pattern="yyyy-MM-dd" />" />
					</div>
			</div>
		...
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:25%;height:25px">
					...
					<div class="controls" style="margin-left:120px" >
						...
						<input name="weight" type="text" style="width:80%" readonly value="${psiTransportOrder.weight}"/> <span class="add-on">kg</span>
						</div>
					</div>
			</div>
			<div class="control-group" style="float:left;width:25%;height:25px">
					...
					<div class="controls" style="margin-left:120px" >
						<div class="input-prepend input-append">
						<input type="text" name="volume" readonly style="width:80%" class="" value="${psiTransportOrder.volume}" id="volume"/> <span class="add-on">m³</span>
						</div>
					</div>
			</div>
			<div class="control-group" style="float:left;width:25%;height:25px">
			...
			<tbody>
				<c:if test="${not empty psiTransportOrder.id }">
					<c:forEach items="${psiTransportOrder.containerItems}"  var="item">
						...
						<select style="width: 90%" class="containerType" name="containerType" disabled>
						...
						<td><input type="text" maxlength="11" readonly style="width: 80%" name="quantity" class="number  containerQuantity" value="${item.quantity}"/></td>
						<td><input type="text" maxlength="100" readonly style="width: 80%" name="itemPrice" class="price containerItemPrice" value="${item.itemPrice}"/></td>
						<td><input type="text" maxlength="50"readonly style="width: 80%" name="remark" value="${item.remark}"/></td>
						</tr>
					</c:forEach>
				</c:if>
			</tbody>
	...
	<blockquote  style="float:left;width:98%;height:25px">
		...
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:30%;height:25px">
					...
					<div class="controls" style="margin-left:100px" >
						<input name="localAmount" type="text" maxlength="10" style="width:95%" readonly class=" price firstAmount" ${fn:contains(canEditStr,'LocalAmount')?'readonly':''} value="${psiTransportOrder.localAmount}"/>
					</div>
			</div>
			...
			<div class="control-group" style="float:left;width:15%;height:25px">
					...
					<div class="controls" style="margin-left:100px" >
						<input  type="text" name="rate1" maxlength="10" style="width:95%" class="rate " readonly value="${psiTransportOrder.rate1}"/>
					</div>
			</div>
			<div class="control-group" style="float:left;width:15%;height:25px">
					<label class="control-label" style="width:80px" >金额1:</label>
					<div class="controls" style="margin-left:100px" >
						<input  type="text" maxlength="10" readonly style="width:95%" class="afterAmount" value="<fmt:formatNumber maxFractionDigits="2" value="${psiTransportOrder.localAmount*psiTransportOrder.rate1}" pattern="#.##" />"/>
					</div>
			</div>
		</div>
		...
		<div style="float:left;width:98%">
			...
			<div class="control-group" style="float:left;width:15%;height:25px">
					...
					<div class="controls" style="margin-left:100px" >
						<input  type="text" name="rate2" maxlength="10" style="width:95%" readonly class="rate " value="${psiTransportOrder.rate2}"/>
					</div>
			</div>
			<div class="control-group" style="float:left;width:15%;height:25px">
		...
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:30%;height:25px">
					...
					<div class="controls" style="margin-left:100px" >
						<input name="dapAmount" type="text"  maxlength="10" style="width:95%" readonly class=" price firstAmount" value="${psiTransportOrder.dapAmount}"/>
					</div>
			</div>
			<div class="control-group" style="float:left;width:20%;height:25px">
			...
			<div class="control-group" style="float:left;width:15%;height:25px">
					...
					<div class="controls" style="margin-left:100px" >
						<input  type="text" name="rate3" maxlength="10" style="width:95%" readonly class="rate " value="${psiTransportOrder.rate3}"/>
					</div>
			</div>
			<div class="control-group" style="float:left;width:15%;height:25px">
			...
			<div style="float:left;width:98%">
				<div class="control-group" style="float:left;width:30%;height:25px">
						...
						<div class="controls" style="margin-left:100px" >
							<input name="otherAmount" type="text"  maxlength="10" style="width:95%" readonly class="price firstAmount" value="${psiTransportOrder.otherAmount}" id="otherAmount"/>
						</div>
				</div>
				<div class="control-group" style="float:left;width:20%;height:25px">
				...
				<div class="control-group" style="float:left;width:15%;height:25px">
						...
						<div class="controls" style="margin-left:100px" >
							<input  type="text" name="rate4" maxlength="10" style="width:95%" readonly class="rate" value="${psiTransportOrder.rate4}"/>
						</div>
				</div>
				<div class="control-group" style="float:left;width:15%;height:25px">
			...
			<div style="float:left;width:98%">
				<div class="control-group" style="float:left;width:30%;height:25px">
						...
						<div class="controls" style="margin-left:100px" >
							<input name="insuranceAmount" type="text"  maxlength="10" style="width:95%" readonly class="price firstAmount" value="${psiTransportOrder.insuranceAmount}"/>
						</div>
				</div>
				<div class="control-group" style="float:left;width:20%;height:25px">
		...
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:15%;height:25px;display:none" id="unitAmount">
				<label class="control-label" style="width:80px">单价:</label>
				<div class="controls" style="margin-left:100px" >
					<input name="unitPrice" type="text" maxlength="10" style="width:85%" class=" price" readonly value="${psiTransportOrder.unitPrice}" id="unitPrice"/>
				</div>
			</div>
			<div class="control-group" style="float:left;width:15%;height:25px">
				<label class="control-label" style="width:80px">总额:</label>
				<div class="controls" style="margin-left:100px" >
					<input type="text" maxlength="10" style="width:85%" readonly class=" price" value="${psiTransportOrder.totalAmount}" id="totalAmount"/>
				</div>
			</div>
		</div>
			...
			<tr>
				...
				<td>
					<select name="sku" class="sku" ${(fn:length(inventorySkus[proKey]) eq 1 && fn:contains(inventorySkus[proKey],item.sku))?'disabled':''}  style="width:98%" ${psiTransportOrder.transportSta ne '0'?'disabled':'' }>
							<c:if test="${!(fn:contains(inventorySkus[proKey],item.sku))}">
				...
				<td><input type="text" maxlength="11" style="width: 80%" name="packQuantity"  class="packQuantity" value="${item.packQuantity}"/></td>
			   ...
			   <c:if test="${'1' eq canCreateFba &&(psiTransportOrder.transportSta eq '0' && psiTransportOrder.transportType eq '1')}">
			   		<td>
			   			<c:if test="${'0' eq item.fbaFlag}">
							<select name="isFba" style="width:95%">
								...
								<option value="1" ${inventory[item.countryCode][item.sku]>=item.quantity?'selected':'' }>是</option>
							</select>
						</c:if>
				...
				<td><input type='text' maxlength='11' style='width: 80%'  name='lowerPrice'  class='lowerPrice'/><input type="hidden" maxlength="11" style="width: 80%" name="productPrice" value="${item.productPrice}" class="productPrice"/></td>
				<td><input type='text' maxlength='11' style='width: 80%'  name='importPrice'  class='importPrice'/></td>
				<td>
				...
				<td><input type="text" maxlength="50" style="width: 80%" name="remark" value="${item.remark}" ${psiTransportOrder.transportSta ne '0'?'readonly':'' }/></td>
				<td><c:if test="${psiTransportOrder.transportSta eq '0' }"><a href="#" class="remove-row"><span class="icon-minus"></span></a></c:if>
				</td>
			</tr>
		</c:forEach>
		</c:if>
