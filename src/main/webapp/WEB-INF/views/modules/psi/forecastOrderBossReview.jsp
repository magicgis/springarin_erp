	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			...
			$(".enterProQuantity").editable({
				...
				success:function(response,newValue){
					if(newValue){
						...
						var oldVal = $(this).text();
						param.itemId = $(this).parent().find(".itemId").val();
						param.promotionBossQuantity = newValue;
						$.get("${ctx}/psi/forecastOrder/updatePromotionBossQuantity?"+$.param(param),function(data){
							if(!(data)){    
								$this.text(oldVal);	
								top.$.jBox.tip("保存失败！", 'info',{timeout:2000});
							}else{
								top.$.jBox.tip("保存促销数量成功！", 'info',{timeout:2000});
							}
						});
					}
					return true;
			...
			$("#isCheck,#isMain,#isNew,#isPriceChange,#lineId,#nameColor,#country").on("change",function(){
				...
				window.location.href="${ctx}/psi/forecastOrder/bossReview?"+$.param(params);
			});
			...
			$(".add-row").on("click",function(e){
	            ...
	            tr.append("<td><input type='text' maxlength='11'  style='width: 85%'   name='promotionBossQuantity'  class='number' /></td>");
	            tr.append("<td></td>");
	            tr.append("<td></td>");
			...
			$("#btnSubmit").on("click",function(){
				top.$.jBox.confirm('您确定要终极审核通过?如果确认将生成采购订单','系统提示',function(v,h,f){
					if(v=='ok'){
					     window.location.href="${ctx}/psi/forecastOrder/bossSave?id="+$("#forecastOrderId").val();
					}
				});
			});
		...
		function goRefresh(){
			...
			window.location.href="${ctx}/psi/forecastOrder/bossReview?id="+id+"&country="+country+"&nameColor="+nameColor+"&isCheck="+isCheck;
		}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#">预测下单终极审核</a></li>
	</ul>
	<form:form id="inputForm" modelAttribute="forecastOrder" action="" method="post" class="form-horizontal">
		...
		<div style="float:left;width:100%;display:inline;">
			<div class="control-group" style="float:left;width:98%;height:30px" >
			   ...
			   &nbsp;&nbsp;
				...
				<label><a class="tips" rel="popover" data-content="最近一个月价格浮动超过20%" href="#"></a></label>
          	...
          	<thead >
				<tr >
				 <th style="width: 3%">序号</th>
				   ...
				   <th style="width: 6%;word-wrap:break-word;word-break:break-all;" >备注</th>
				   <th style="width: 6%;word-wrap:break-word;word-break:break-all;" >审批备注</th>
				   <th style="width: 6%;word-wrap:break-word;word-break:break-all;" >终极备注</th>
		...
		<colgroup>
               <col style="width: 3%"/><col style="width: 10%"/><col style="width: 5%"/><col style="width: 4%"/><col style="width: 4%"/><col style="width: 4%"/><col style="width: 4%"/><col style="width: 6%"/><col style="width: 4%"/><col style="width: 4%"/>
               <col style="width: 4%"/><col style="width: 4%"/><col style="width: 4%"/><col style="width: 4%"/><col style="width: 4%"/><col style="width: 4%"/><col style="width: 4%"/><col style="width: 4%"/>
       		   <col style="width: 4%"/><col style="width: 4%"/><col style="width: 6%"/><col style="width: 6%"/><col style="width: 6%"/><col style="width: 4%"/><col style="width: 6%"/>
         	 </colgroup>
			...
			<c:forEach items="${forecastOrder.productMap}" var="productMap" varStatus="j">
			    ...
			    <c:choose>
			    	<c:when test="${i.index==0}">
						...
						<td><input type="hidden" class="itemId" value="${item.id}" /> <a href="#" class="enterRemark"  data-type="text" data-pk="1" data-title="Enter Remark" data-value="${item.bossRemark}">${item.bossRemark}</a></td>
						<td >
