...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			var cnt="<form  id='inputForm' action='${ctx}/psi/psiOutOfStockInfo/findEuGapInfo' method='post' > "+
			 ...
			 $("#expExcel").click(function(){
				 $("#expExcel").attr("href","${ctx}/psi/psiOutOfStockInfo/exportEuDetail?forecastType=0&country="+$("#country").val());
			});
			 $("#expExcel2").click(function(){
				 $("#expExcel2").attr("href","${ctx}/psi/psiOutOfStockInfo/exportEuDetail?forecastType=1&country="+$("#country").val());
			});	
	...
	<ul class="nav nav-tabs">
		<c:forEach items="${fns:getDictList('platform')}" var="dic" varStatus="i">
			<c:if test="${dic.value ne 'com.unitek'}">
				<li><a class="countryHref"  href='${ctx}/psi/psiOutOfStockInfo/findGapInfo?country=${dic.value}' key="${dic.value}">${dic.label}</a></li>
			</c:if>
		</c:forEach>
		<li  class="active"><a href="${ctx}/psi/psiOutOfStockInfo/findEuGapInfo">欧洲</a></li>	
			...
			<c:forEach items="${gapMap}" var="gap"> 
			  <c:forEach items="${gapMap[gap.key]}" var="gapType"> 
			     <c:forEach items="${gapMap[gap.key][gapType.key]}" var="nameType"> 
			        <c:if test="${'de' eq country &&'0' eq powerMap[gapType.key] }">
				      <tr> <c:set value='${gapType.key }_${country }' var="name"/>
				           <td style="text-align: center;vertical-align: middle;">
				               <span style="visibility:hidden;">${'0' eq gap.key?'0':('1' eq gap.key?'1':('2' eq gap.key?'3':'2')) }</span> 
				                ${'0' eq gap.key?'销售预测':('1' eq gap.key?'周日销':('2' eq gap.key?'周日销(安)':'销售预测(安)')) }
				            </td>
			               <td style="text-align: center;vertical-align: middle;"><a href="${ctx}/psi/psiInventory/productInfoDetail?productName=${gapType.key}" target="_blank">${gapType.key}</a>
			                   ...
			                   <c:if test="${'2' eq tranMap[gapType.key]['eu']}"><span class="icon-plane"></span></c:if>
			               </td>
