...
<html>
<head>
	<title>Product AddedMonth Manager</title>
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/psi/productEliminate/addedMonthlist">上架日期</a></li>
		<li class="dropdown">
	...
	<table id="contentTable" class="table  table-bordered table-condensed">
		...
		<c:forEach items="${list}" var="productName" varStatus="i">
			<tr>
				...
				<td>
					<c:if test="${not empty products[productName]['de'][0]}">${fn:replace(products[productName]['de'][0],' 00:00:00','')}</c:if>
				</td>
				<td>
					<c:if test="${not empty products[productName]['uk'][0]}">${fn:replace(products[productName]['uk'][0],' 00:00:00','')}</c:if>
				</td>
				<td>
					<c:if test="${not empty products[productName]['it'][0]}">${fn:replace(products[productName]['it'][0],' 00:00:00','')}</c:if>
				</td>
				<td>
					<c:if test="${not empty products[productName]['es'][0]}">${fn:replace(products[productName]['es'][0],' 00:00:00','')}</c:if>
				</td>
				<td>
					<c:if test="${not empty products[productName]['fr'][0]}">${fn:replace(products[productName]['fr'][0],' 00:00:00','')}</c:if>
				</td>
				<td>
					<c:if test="${not empty products[productName]['jp'][0]}">${fn:replace(products[productName]['jp'][0],' 00:00:00','')}</c:if>
				</td>
				<td>
					<c:if test="${not empty products[productName]['com'][0]}">${fn:replace(products[productName]['com'][0],' 00:00:00','')}</c:if>
				</td>
				<td>
					<c:if test="${not empty products[productName]['ca'][0]}">${fn:replace(products[productName]['ca'][0],' 00:00:00','')}</c:if>
				</td>
				<td>
					<c:if test="${not empty products[productName]['mx'][0]}">${fn:replace(products[productName]['mx'][0],' 00:00:00','')}</c:if>
				</td>
				<%--<td>
					<a class="btn btn-success btn-small" href="${ctx}/psi/productEliminate/setStatus?productName=${productName}&flag=3">修改</a>
				</td>--%>
			</tr>
		</c:forEach>
		</tbody>
