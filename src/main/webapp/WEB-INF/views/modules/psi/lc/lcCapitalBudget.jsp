	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			...
			$("#dataTable tbody tr").each(function(){
				...
				var td16=td15.next();
				if(td16.text().trim()!=""){
					unOver9 =unOver9+parseFloat(td16.text().replace(/,/g,""));
				}
				
				var td17=td16.next();
				if(td17.text().trim()!=""){
					unReceived9 =unReceived9+parseFloat(td17.text().replace(/,/g,""));
				}
				
				var td18=td17.next();
				if(td18.text().trim()!=""){
					total =total+parseFloat(td18.text().replace(/,/g,""));
				}
			});
			...
			$("#unOver9").append("<b><span style='color:#EAC100;padding-left:5px;font-size:16px'>¥</span>"+formatNumber(unOver9,0,true)+"</b>");
			$("#unReceived9").append("<b><span style='color:#EAC100;padding-left:5px;font-size:16px'>¥</span>"+formatNumber(unReceived9,0,true)+"</b>");
		...
		function overDetail(supplierId){
			//window.location.href="${ctx}/psi/lcPurchaseOrder/overDetail?supplierId="+supplierId;
			window.location.href="${ctx}/psi/lcPurchaseOrder/overLadDetail?supplierId="+supplierId;
		}
...
<body style="width:400px;overflow:auto;">
    ...
    <shiro:hasPermission name="psi:purchaseOrder:accounts">
		...
		<li class="active"><a href="${ctx}/psi/lcPurchaseOrder/capitalBudget">(理诚)资金分配计划</a></li>
	...
	<form:form id="searchForm" modelAttribute="lcPurchaseOrder" action="${ctx}/psi/lcPurchaseOrder/capitalBudget" method="post" class="breadcrumb form-search" cssStyle="height: 30px;">
			<label>供应商：</label>
	...
	<div style="${empty mobilCss?'overflow-x:auto':mobilCss}">
	<table id="dataTable" class="table table-bordered table-condensed">
		<thead>
		<tr>
			<th width="4%" rowspan="2">供应商</th><th width="4%"  rowspan="2">已逾期金额</th>
			<c:set value="" var="lastMonth" />
			<c:forEach items="${weeks}" var="week" varStatus="i">
				<c:choose>
					<c:when test="${i.index <8 }"><th width="8%" colspan="2">${weekStartMap[week]}</th></c:when>
					<c:when test="${i.index ==8}"> <c:set var="lastMonth"  value="${week}" /></c:when>
					<c:when test="${i.index ==11}"><c:set  var="lastMonth" value="${lastMonth}W 至  ${week}W"/></c:when>
				</c:choose>
			</c:forEach>
			<th width="8%" colspan="2">${lastMonth}</th>
			<th width="4%"  rowspan="2">Total</th>
		</tr>
		<tr>
			<c:forEach items="${weeks}" var="week" varStatus="i">
				<c:if test="${i.index <9 }">
					<th width="4%">尾款</th><th width="4%">未收货</th>
				</c:if>
			</c:forEach>
		</tr>
		</thead>
		...
		<c:forEach items="${supplierMap}" var="supplierEntry" varStatus="i">
			<tr>
				...
				<c:set  var="lastMonthLading" value="0"/>
				<c:set  var="lastMonthOrder" value="0"/>
				
				<c:if test="${not empty ladingMap[supplierEntry.key] || not empty orderMap[supplierEntry.key]}">
					<td class="overAmount">
					<a href="${ctx}/psi/lcPurchaseOrder/overLadDetail?supplierId=${supplierEntry.key}"><fmt:formatNumber value="${ladingMap[supplierEntry.key]['over']}" pattern=",###"  /></a>
					<c:set var="supplierTotal" value="${supplierTotal+ladingMap[supplierEntry.key]['over']}" />
					</td>
					<c:forEach items="${weeks}" var="week" varStatus="i">
						<c:choose>
							<c:when test="${i.index<8}">
								<td>
							...
							<c:otherwise>
								<c:set  var="lastMonthLading" value="${lastMonthLading+ladingMap[supplierEntry.key][week]}"/>
								<c:set  var="lastMonthOrder" value="${lastMonthOrder+orderMap[supplierEntry.key][week]}"/>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					<c:set var="supplierTotal" value="${supplierTotal+lastMonthLading+lastMonthOrder}" />
					<td><fmt:formatNumber value="${lastMonthLading}" pattern=",###"  /></td>
					<td><fmt:formatNumber value="${lastMonthOrder}" pattern=",###"  /></td>   
				</c:if>
