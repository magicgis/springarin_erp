			...
			$("#inputForm").validate({
				submitHandler: function(form){
					if($(".MultiFile-title").size()==0){
						top.$.jBox.tip("请选择供应商税务发票！", 'info',{timeout:3000});
...
<body>
	...
	<form:form id="inputForm" modelAttribute="psiTransportPayment" action="${ctx}/psi/lcPsiTransportPayment/uploadBillSave" method="post" class="form-horizontal" enctype="multipart/form-data">
		...
		<div style="float:left;width:98%;">
			...
			<div class="control-group"  style="float:left;width:70%;height:25px" >
					...
					<div class="controls" >
					<span>
						<input readonly="readonly" value="${psiTransportPayment.accountType}" type="text" />
					</span>
					</div>
				</div>
		</div>
	   <div style="float: left"><blockquote><p style="font-size: 14px">付款项信息</p></blockquote></div><div style="float: left" id=errorsShow></div>
		...
		<div class="control-group" style="float:left;width:98%;">
			...
			<c:forEach items="${fileMap}" var="fileMap">
					<c:forEach items="${fn:split(fileMap.value,',')}" var="attchment" varStatus="j">
						<a target="_blank" href="<c:url value='/data/site/psi/lcPsiTransport/${attchment}'/>">${fileMap.key}_${j.index+1}</a>
						&nbsp;&nbsp;&nbsp;  
					</c:forEach>
			</c:forEach>
			</div>
		...
		<blockquote style="float:left;width:98%;">
			<p style="font-size: 14px">上传供应商税务发票</p>
		</blockquote>
		<div class="control-group" style="float:left;width:98%;">
			<div class="controls">
