...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/psi/lcPsiTransportRevise/">(理诚)运单付款修正列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="lcPsiTransportRevise" action="${ctx}/psi/lcPsiTransportRevise/" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<div style="height: 100px;line-height: 40px">
			<div style="height: 40px;">
			<label>申请日期：</label><input style="width: 100px" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',onpicked:function(){$('#searchForm').submit();return true}});" readonly="readonly"  class="Wdate" type="text" name="applyDate" value="<fmt:formatDate value="${lcPsiTransportRevise.applyDate}" pattern="yyyy-MM-dd"/>" class="input-small" id="start"/>
			&nbsp;&nbsp;-&nbsp;&nbsp;<input style="width: 100px" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',onpicked:function(){$('#searchForm').submit();return true}});" readonly="readonly"  class="Wdate" type="text" name="sureDate" value="<fmt:formatDate value="${lcPsiTransportRevise.sureDate}" pattern="yyyy-MM-dd" />" id="end" class="input-small"/>
				&nbsp;&nbsp;&nbsp;&nbsp;
		...
		&nbsp;<input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>
			...
			<select style="width:150px;" id="supplier" name="supplier.id">
				<option value="" ${lcPsiTransportRevise.supplier eq '' ?'selected':''}><spring:message code="amazon_order_tips4"/></option>
				<c:forEach items="${suppliers}" var="supplier" varStatus="i">
					 <option value='${supplier.id}' ${lcPsiTransportRevise.supplier.id eq supplier.id ?'selected':''}>${supplier.nikename}</option>;
				</c:forEach>
			</select>
			&nbsp;&nbsp;&nbsp;&nbsp;
			<label>付款状态：</label>
			<form:select path="reviseSta" style="width: 200px" id="reviseSta">
				...
				<option value="0" ${lcPsiTransportRevise.reviseSta eq '0' ?'selected':''} >申请</option>
				<option value="3" ${lcPsiTransportRevise.reviseSta eq '3' ?'selected':''} >已审核</option>
				<option value="5" ${lcPsiTransportRevise.reviseSta eq '5' ?'selected':''} >完成</option>
				<option value="8" ${lcPsiTransportRevise.reviseSta eq '8' ?'selected':''} >取消</option>
			</form:select>
			</div>
		</div>
	</form:form>
	...
	<table id="contentTable" class="table table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="psiTransportRevise">
			<tr>
				...
				<td><a href="${ctx}/psi/lcPsiTransportRevise/view?id=${psiTransportRevise.id}">${psiTransportRevise.paymentNo}</a></td>
				...
				<c:if test="${psiTransportRevise.reviseSta eq '0' }" >
					<shiro:hasPermission name="psi:tranRevise:review">
						<a  class="btn btn-small"  href="${ctx}/psi/lcPsiTransportRevise/review?id=${psiTransportRevise.id}">审核修正付款</a>
					</shiro:hasPermission>
					<a  class="btn btn-small"  href="${ctx}/psi/lcPsiTransportRevise/cancel?id=${psiTransportRevise.id}" onclick="return confirmx('确认要取消该修正付款项吗？', this.href)">取消</a>
				</c:if>
				
				<c:if test="${psiTransportRevise.reviseSta eq '3' }" >
					<shiro:hasPermission name="psi:tranRevise:sure">
						<a  class="btn btn-small"  href="${ctx}/psi/lcPsiTransportRevise/sure?id=${psiTransportRevise.id}">确认修正付款</a>
					</shiro:hasPermission>
				</c:if>
				<c:if test="${psiTransportRevise.totalAmount>0}">
					<a  class="btn btn-small"  href="${ctx}/psi/lcPsiTransportRevise/printPayment?id=${psiTransportRevise.id}">付款申请单</a>
				</c:if>
				</td>
			</tr>
