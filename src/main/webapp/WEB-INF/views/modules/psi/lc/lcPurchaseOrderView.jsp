			...
			<table id="contentTable" class="table table-striped table-bordered table-condensed">
				...
				<tbody>
					<tr>
						...
						<td>${purchaseOrder.salesUserName}</td>
						...
						<shiro:hasPermission name="psi:product:viewPrice">
						 ...
						 <shiro:lacksPermission name="psi:product:viewPrice">
		       				<c:if test="${fns:getUser().id eq purchaseOrder.merchandiser.id}">
								...
								<td>
								<c:if test="${not empty purchaseOrder.piFilePath }">
									<a  target="_blank" href="${ctx}/psi/lcPurchaseOrder/printPi?id=${purchaseOrder.id}">查看</a>
								</c:if>
								</td>
		       				</c:if>
		       			</shiro:lacksPermission>
		...
		<thead>
			<tr>
				   ...
				   <shiro:hasPermission name="psi:product:viewPrice">
				    ...
				    <shiro:lacksPermission name="psi:product:viewPrice">
		       			<c:if test="${fns:getUser().id eq purchaseOrder.merchandiser.id}"><th style="width: 5%">单价</th>	</c:if>
		       		</shiro:lacksPermission>
			...
			<c:forEach items="${purchaseOrder.orderItemsMap}" var="itemEntry" >
				 <c:forEach items="${itemEntry.value.items}" var="item" varStatus="i">
				 	  ...
				 	  <c:if test="${i.index==0}">
						...
						<a href="${ctx}/psi/lcProduct/genBarcode?country=${item.barcodeInstans.productPlatform}&type=${item.barcodeInstans.barcodeType}&productName=${item.barcodeInstans.barcodeProductName}&barcode=${item.barcodeInstans.barcode}" target="_blank" class="btn btn-warning" >${item.barcodeInstans.barcode}</a>
						</c:if>
						</td>
						<td>
		...
		<thead>
			<tr>
				...
				<c:forEach items="${itemEntry.value.items}" var="billItem" varStatus="i">
				 	  <tr>
				 	  <c:if test="${i.index==0}">
				 	  <td rowspan="${fn:length(itemEntry.value.items)}" style="text-align: center;vertical-align: middle;"><a href="${ctx}/psi/lcPsiLadingBill/${itemEntry.value.billSta=='0'?'sure':'view'}?id=${billItem.ladingBill.id}">${billItem.ladingBill.billNo}</a></td>
				 	  </c:if>
		...
		<table id="contentTable" class="table table-striped table-bordered table-condensed">
			...
			<tbody>
				<c:forEach items="${purchaseOrder.paymentsMap}" var="pay" >
				 	  <tr>
				 	  <td><a href="${ctx}/psi/lcPurchasePayment/view?id=${pay.value.id}">${pay.value.paymentNo}</a></td>   
						<td>
