...
<head>
	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			if(!(top)){
				...
				$(".containerQuantity").live("blur",function(){
					var tbody =$(this).parent().parent().parent();
					var tranAmount=0;
					tbody.find("tr").each(function(){
						if($(this).find(".containerItemPrice").val()!=''&&$(this).find(".containerQuantity").val()!=''){
							tranAmount=tranAmount+parseFloat($(this).find(".containerItemPrice").val())*parseInt($(this).find(".containerQuantity").val());
						}
					});
					$("input[name='tranAmount']").val(toDecimal(tranAmount));
					if($(this).parent().parent().find(".containerItemPrice").val()!=''){
						//光标移动到tranAmount上
						changeTranAmount();
					}
				...
				$(".containerItemPrice").live("blur",function(){
					var tbody =$(this).parent().parent().parent();
					var tranAmount=0;
					tbody.find("tr").each(function(){
						if($(this).find(".containerItemPrice").val()!=''&&$(this).find(".containerQuantity").val()!=''){
					...
					$("input[name='tranAmount']").val(tranAmount);
					if($(this).parent().parent().find(".containerQuantity").val()!=''){
						changeTranAmount();
					}
				...
				$("#unitPrice").on("blur",function(){
					...
					$("input[name='tranAmount']").val(toDecimal(parseFloat(unitPrice*volume)));
					changeTranAmount();
				});
				
				$("#volume").on("blur",function(){
					if($("select#oceanModel").val()=='LCL'){
					...
					if($(this).val()!=''){
						...
						if(div.find(".firstAmount").val()!=''){
							...
							parentDiv.find(".afterAmount").each(function(){
								...
								if(innerDiv.find(".firstAmount").val()!=''&&$(this).val()!=''){
									total=total+parseFloat($(this).val());
								}
							});
							
							$("#totalAmount").val(toDecimal(parseFloat(total)));
						}
					}
				...
				$(".currency").on("change",function(){
					...
					if($(this).val()){
						tr.find(".rate").val(rateMap[$(this).val()]).blur();
					}
				});
				...
				$("#oceanModel").on("change",function(){
					if($(this).val()=='FCL'){
						$("#showContainer").css("display","block");
						$("#unitAmount").css("display","none");
						if($("#orderId").val()==''||!$("#containerTable tbody tr").size()>0){
							var tbody=$("#containerTable tbody");
							var tr=$("<tr></tr>");
							tr.append("<td><select style='width: 90%' class='containerType' name='containerType' ><c:forEach items='${containerTypes }' var='containerType'><option value='${containerType}'>${containerType}</option></c:forEach></select>");
							tr.append("<td><input type='text' maxlength='11' style='width: 80%' name='quantity' class='number containerQuantity' /></td>");
							tr.append("<td><input type='text' maxlength='11' style='width: 80%' name='itemPrice' class='price containerItemPrice'/></td>");
							tr.append("<td><input type='text' maxlength='50' style='width: 80%' name='remark' /></td>");
							tr.append("<td><a href='#' class='remove-row-container'><span class='icon-minus'></span>删除集装箱</a></td>");
							tbody.append(tr);
							tr.find(".containerType").select2();
						}
					}else{
				...
				$(".remove-row-container").live("click",function(){
					 if($('#containerTable tbody tr').size()>1){
						var tr = $(this).parent().parent();
						tr.remove();
					}
				});
				
				$("#add-row-container").on("click",function(){
					var tbody=$("#containerTable tbody");
					var tr=$("<tr></tr>");
					tr.append("<td><select style='width: 90%' class='containerType' name='containerType' ><c:forEach items='${containerTypes }' var='containerType'><option value='${containerType}'>${containerType}</option></c:forEach></select>");
					tr.append("<td><input type='text' maxlength='11' style='width: 80%' name='quantity' class='number containerQuantity' /></td>");
					...
					submitHandler: function(form){
						...
						if(model=='FCL'){
							var keyStr1="";
							 ...
							 var twoStr1="";
							$("#containerTable tbody tr").each(function(){
								var curkeyStr1=$(this).find("select.containerType").children("option:selected").text();
								if(keyStr1.indexOf(curkeyStr1+",")>=0){
									twoStr1=curkeyStr1;
									flag1 = 2;
									return false;
								}else{
									keyStr1=keyStr1+curkeyStr1+",";
								};
								var quantity1 = $(this).find("input[name='quantity']").val();
								if(quantity1==0){
									zeroFlag1=2;
									twoStr1=curkeyStr1;
									return false;
							...
							if(flag1==2){
								top.$.jBox.tip("相同集装箱类型只能有一条记录!!!  "+twoStr1, 'info',{timeout:3000});
								return false;
							}
							
							//if(zeroFlag1==2){
							//	top.$.jBox.tip("集装箱数量必须大于0!!! "+twoStr1, 'info',{timeout:3000});
							//	return false;
							//}
						}
						...
						$("#containerTable tbody tr").each(function(i,j){
							$(j).find("select").each(function(){
								if($(this).attr("name")){
									$(this).attr("name","containerItems"+"["+i+"]."+$(this).attr("name"));
								}
							});
							$(j).find("input[type!='']").each(function(){
								if($(this).attr("name")){
	...
	<body>
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="#">(理诚)${empty psiTransportOrder.id ?'新建':'编辑'}海运运单</a></li>
		</ul><br/>
		<form:form id="inputForm" modelAttribute="psiTransportOrder" action="${ctx}/psi/lcPsiTransportOrder/editSave" method="post" class="form-horizontal" enctype="multipart/form-data">
			<input type="hidden" name="id" 			  value="${psiTransportOrder.id}" id="orderId"/>
			<input type="hidden" name="model" 		  value="${psiTransportOrder.model}"/>
			...
			<input type="hidden" name="oldContainerIds"    value="${psiTransportOrder.oldContainerIds}"/>
			...
			<input type="hidden" class="containerFlag" value="${fn:length(psiTransportOrder.containerItems)==0}"/>
			...
			<blockquote>
				...
				<div style="float:left;width:98%">
					<div class="control-group" style="float:left;width:25%;height:25px">
						<label class="control-label" style="width:100px">${psiTransportOrder.model eq '1'?'海运':(psiTransportOrder.model eq '4'?'公路':'铁路')}模式:</label>
						<div class="controls" style="margin-left:120px" >
							<select name="oceanModel" style="width: 100%" id="oceanModel"  ${(not empty canEditStr && ('OtherAmount,' ne canEditStr||'OtherAmount1,' ne canEditStr||'OtherAmount,OtherAmount1,' ne canEditStr||'OtherAmount1,OtherAmount,' ne canEditStr))?'disabled':''}>
								<option value="FCL" ${psiTransportOrder.oceanModel eq 'FCL'?'selected':''}>FCL</option>
								<option value="LCL" ${psiTransportOrder.oceanModel eq 'LCL'?'selected':''}>LCL</option>
							</select>
						</div>
					</div>
					<div class="control-group" style="float:left;width:25%;height:25px">
		...
		<div style="float:left;width:98%">
			<c:if test="${psiTransportOrder.transportType ne '2'}">
					<div class="control-group" style="float:left;width:25%;height:25px">
			...
			<c:if test="${psiTransportOrder.transportType eq '2'}">
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
					<label class="control-label" style="width:100px">起运${psiTransportOrder.model eq '1'?'港口':(psiTransportOrder.model eq '4'?'地':'火车站')}:</label>
					<div class="controls" style="margin-left:120px" >
						<input name="orgin" type="text" maxlength="10" style="width:95%" readonly value="${psiTransportOrder.orgin}"/>
					</div>
			</div>
			<div class="control-group" style="float:left;width:25%;height:25px">
					<label class="control-label" style="width:100px">目的${psiTransportOrder.model eq '1'?'港口':(psiTransportOrder.model eq '4'?'地':'火车站')}:</label>
					<div class="controls" style="margin-left:120px" >
						<input name="destination" type="text" maxlength="10" style="width:90%" readonly value="${psiTransportOrder.destination}"/>
					</div>
			</div>
		</div>
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:25%;height:25px">
					<label class="control-label" style="width:100px">${psiTransportOrder.model eq '1'?'船舶':(psiTransportOrder.model eq '4'?'公路':'铁路')}公司:</label>
					<div class="controls" style="margin-left:120px" >
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
					<label class="control-label" style="width:100px">离${psiTransportOrder.model eq '1'?'港':'站'}日期:</label>
					<div class="controls" style="margin-left:120px" >
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
					<label class="control-label" style="width:100px">预计到${psiTransportOrder.model eq '1'?'港':'站'}日期:</label>
					<div class="controls" style="margin-left:120px" >
		...
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:25%;height:25px">
					...
					<div class="controls" style="margin-left:120px" >
						...
						<input name="weight" type="text" style="width:80%" class="" value="${psiTransportOrder.weight}"/> <span class="add-on">kg</span>
						</div>
					</div>
			</div>
			<div class="control-group" style="float:left;width:25%;height:25px;display:none" id="unitAmount">
				<label class="control-label" style="width:100px">单价:</label>
				<div class="controls" style="margin-left:120px" >
					<input name="unitPrice" type="text" maxlength="10" style="width:95%" class=" price" ${fn:contains(canEditStr,'TranAmount')&&psiTransportOrder.oceanModel eq 'LCL'?'readonly':''} value="${psiTransportOrder.unitPrice}" id="unitPrice"/>
				</div>
			</div>
			<div class="control-group" style="float:left;width:25%;height:25px">
					...
					<div class="controls" style="margin-left:120px" >
						<div class="input-prepend input-append">
						<input type="text" name="volume" ${fn:contains(canEditStr,'TranAmount')&&psiTransportOrder.oceanModel eq 'LCL'?'readonly':''} style="width:80%" class="" value="${psiTransportOrder.volume}" id="volume"/> <span class="add-on">m³</span>
						</div>
					</div>
			</div>
			<div class="control-group" style="float:left;width:25%;height:25px">
			...
			<div style="float:left; width:98%;display:none" id="showContainer">
			<blockquote  style="float:left;">
				<div style="float: left"><p style="font-size: 14px">集装箱信息</p></div>
			</blockquote>
			
			
			<div  style="font-size: 14px;margin: 5px 100px 5px 0px;float:right;display: ${fn:contains(canEditStr,'TranAmount')&&psiTransportOrder.oceanModel eq 'FCL'?'none':''}"><a href="#" id="add-row-container"><span class="icon-plus"></span>增加集装箱</a></div>
			...
			<tbody>
				<c:if test="${not empty psiTransportOrder.id && fn:length(psiTransportOrder.containerItems)>0}">
					<c:forEach items="${psiTransportOrder.containerItems}"  var="item">
						<tr>
						<td>
						<input type="hidden" name="id" value="${item.id}"/>
						<select style="width: 90%" class="containerType" name="containerType" ${fn:contains(canEditStr,'TranAmount')&&psiTransportOrder.oceanModel eq 'FCL'?'disabled':''}>
							<c:forEach items="${containerTypes }" var="containerType">
								<option value="${containerType}" ${ containerType eq item.containerType ?'selected':''}>${containerType}</option>
							</c:forEach>
						</select>
						</td>
						<td><input type="text" maxlength="11"  ${fn:contains(canEditStr,'TranAmount')&&psiTransportOrder.oceanModel eq 'FCL'?'readonly':''} style="width: 80%" name="quantity" class="number  containerQuantity" value="${item.quantity}"/></td>
						<td><input type="text" maxlength="100" ${fn:contains(canEditStr,'TranAmount')&&psiTransportOrder.oceanModel eq 'FCL'?'readonly':''} style="width: 80%" name="itemPrice" class="price containerItemPrice" value="${item.itemPrice}"/></td>
						<td><input type="text" maxlength="50" ${fn:contains(canEditStr,'TranAmount')&&psiTransportOrder.oceanModel eq 'FCL'?'readonly':''} style="width: 80%" name="remark" value="${item.remark}"/></td>
						<td><a href="#" class="remove-row-container" style="display:${fn:contains(canEditStr,'TranAmount')&&psiTransportOrder.oceanModel eq 'FCL'?'none':''}"><span class="icon-minus"></span>删除集装箱</a>	</td>
						</tr>
					</c:forEach>
				</c:if>
			</tbody>
	...
	<blockquote  style="float:left;width:98%;height:25px">
			...
			<strong >海运Tips：FCL:运输运费=集装箱单价x集装箱数量;;;LCL:运输费用=体积x单价 ;;;转换后金额=费用x汇率;;;总金额=各转换后金额累加;;;箱数=数量÷装箱数</strong></div>   
		</blockquote>
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:30%;height:25px">
					...
					<div class="controls" style="margin-left:100px" >
						<input name="localAmount" type="text" maxlength="10" style="width:95%" class=" price firstAmount" ${fn:contains(canEditStr,'LocalAmount')?'readonly':''} value="${psiTransportOrder.localAmount}"/>
					</div>
			</div>
			<div class="control-group" style="float:left;width:20%;height:25px">
					<label class="control-label" style="width:80px" >Currency1:</label>
					<div class="controls" style="margin-left:100px" >
					<select name="currency1" class="currency" style="width:95%" ${fn:contains(canEditStr,'LocalAmount')?'disabled':''}>
						<option value=""></option>
			...
			<div class="control-group" style="float:left;width:20%;height:25px">
					...
					<div class="controls" style="margin-left:100px">
						<select name="vendor1.id"  style="width:95%" ${fn:contains(canEditStr,'LocalAmount')?'disabled':''}>
						<option value=""></option>
			...
			<div class="control-group" style="float:left;width:15%;height:25px">
					...
					<div class="controls" style="margin-left:100px" >
						<input  type="text" name="rate1"  maxlength="10" style="width:95%" class="rate" ${fn:contains(canEditStr,'LocalAmount')?'readonly':''} value="${psiTransportOrder.rate1}"/>
					</div>
			</div>
			<div class="control-group" style="float:left;width:15%;height:25px">
		...
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:30%;height:25px">
					...
					<div class="controls" style="margin-left:100px" >
						<input name="tranAmount" type="text" maxlength="10" style="width:95%" readonly class=" price firstAmount" value="${psiTransportOrder.tranAmount}"/>
					</div>
			</div>
			<div class="control-group" style="float:left;width:20%;height:25px">
					<label class="control-label" style="width:80px">Currency2:</label>
					<div class="controls" style="margin-left:100px" >
						<select name="currency2" class="currency" style="width:95%" ${fn:contains(canEditStr,'TranAmount')?'disabled':''}>
						<option value=""></option>
						<c:forEach items="${currencys}" var="currency">
		...
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:30%;height:25px">
					...
					<div class="controls" style="margin-left:100px" >
						<input name="dapAmount" type="text"  maxlength="10" style="width:95%" ${(fn:contains(canEditStr,'DapAmount')&&psiTransportOrder.vendor3.id ne '18')?'readonly':''} class=" price firstAmount" value="${psiTransportOrder.dapAmount}"/>
					</div>
			</div>
			<div class="control-group" style="float:left;width:20%;height:25px">
					<label class="control-label" style="width:80px" >Currency3:</label>
					<div class="controls" style="margin-left:100px" >
						<select name="currency3" class="currency" style="width:95%" ${fn:contains(canEditStr,'DapAmount')?'disabled':''}>
						<option value=""></option>
						<c:forEach items="${currencys}" var="currency">
			...
			<div class="control-group" style="float:left;width:15%;height:25px">
					...
					<div class="controls" style="margin-left:100px" >
						<input  type="text" name="rate3" maxlength="10" style="width:95%" ${fn:contains(canEditStr,'DapAmount')?'readonly':''} class="rate" value="${psiTransportOrder.rate3}"/>
					</div>
			</div>
			<div class="control-group" style="float:left;width:15%;height:25px">
			...
			<div style="float:left;width:98%">
				<div class="control-group" style="float:left;width:30%;height:25px">
						...
						<div class="controls" style="margin-left:100px" >
							<input name="otherAmount" type="text"  maxlength="10" style="width:20%" ${fn:contains(canEditStr,'OtherAmount')?'readonly':''} class="price firstAmount" value="${psiTransportOrder.otherAmount}" id="otherAmount"/>
							Mark:<input name="otherRemark" type="text"  ${fn:contains(canEditStr,'OtherAmount')?'readonly':''} style="width:50%" value="${psiTransportOrder.otherRemark}" />
						</div>
				</div>
				<div class="control-group" style="float:left;width:20%;height:25px">
						<label class="control-label" style="width:80px" >Currency4:</label>
						<div class="controls" style="margin-left:100px" >
							<select name="currency4" class="currency" style="width:95%" ${fn:contains(canEditStr,'OtherAmount')?'disabled':''}>
							<option value="" ></option>
							<c:forEach items="${currencys}" var="currency">
				...
				<div class="control-group" style="float:left;width:15%;height:25px">
						...
						<div class="controls" style="margin-left:100px" >
							<input  type="text" name="rate4" maxlength="10" style="width:95%" ${fn:contains(canEditStr,'OtherAmount')?'readonly':''} class="rate" value="${psiTransportOrder.rate4}"/>
						</div>
				</div>
				<div class="control-group" style="float:left;width:15%;height:25px">
				...
				<div class="control-group" style="float:left;width:30%;height:25px">
						...
						<div class="controls" style="margin-left:100px" >
							<input name="otherAmount1" type="text"  ${fn:contains(canEditStr,'OtherAmount1')?'readonly':''} maxlength="10" style="width:20%" class="price firstAmount" value="${psiTransportOrder.otherAmount1}" id="otherAmount1"/>
							Mark:<input name="otherRemark1" type="text"  ${fn:contains(canEditStr,'OtherAmount')?'readonly':''} style="width:50%" value="${psiTransportOrder.otherRemark1}" />
						</div>
				</div>
				<div class="control-group" style="float:left;width:20%;height:25px">
						<label class="control-label" style="width:80px" >Currency7:</label>
						<div class="controls" style="margin-left:100px" >
							<select name="currency7" class="currency" style="width:95%"  ${fn:contains(canEditStr,'OtherAmount1')?'disabled':''}>
							<option value=""></option>
							<c:forEach items="${currencys}" var="currency">
			...
			<div style="float:left;width:98%">
				<div class="control-group" style="float:left;width:30%;height:25px">
						...
						<div class="controls" style="margin-left:100px" >
							<input name="insuranceAmount" type="text"  maxlength="10" style="width:95%" ${fn:contains(canEditStr,'InsuranceAmount')?'readonly':''} class="price firstAmount" value="${psiTransportOrder.insuranceAmount}"/>
						</div>
				</div>
				<div class="control-group" style="float:left;width:20%;height:25px">
		...
		<div style="float:left;width:98%">
			...
			<div class="control-group" style="float:left;width:28%;height:25px">
				<label class="control-label" style="width:80px">进口总额:</label>
				<div class="controls" style="margin-left:100px" >
					<input type="text" maxlength="10" style="width:85%" id="totalImportAmount"  readonly class=" price" value="${psiTransportOrder.totalImportAmount}"/>
				</div>
			</div>
		</div>
		...
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:15%;height:25px">
				...
				<div class="controls" style="margin-left:100px" >
					<input type="text" maxlength="10" style="width:85%" class=" price" value="${psiTransportOrder.totalAmount}" id="totalAmount"/>
				</div>
			</div>
			...
			<div class="control-group" style="float:left;width:20%;height:25px">
				...
				<div class="controls" style="margin-left:100px" >
					<input type="text" maxlength="10" style="width:85%"  class="price" name="declareAmount" value="${psiTransportOrder.declareAmount}"/>
				</div>
			</div>
			<div class="control-group" style="float:left;width:20%;height:25px">
				<label class="control-label" style="width:80px">退税金额:</label>
				<div class="controls" style="margin-left:100px" >
					<input type="text" maxlength="10" style="width:85%"  class="price" name="taxRefundAmount" value="${psiTransportOrder.taxRefundAmount}"/>
				</div>
			</div>
			<div class="control-group" style="float:left;width:25%;height:25px">
				<label class="control-label" style="width:80px">报关单号:</label>
				<div class="controls" style="margin-left:100px" >
					<input type="text" style="width:90%"   name="declareNo" value="${psiTransportOrder.declareNo}"/>
				</div>
			</div>
			<div class="control-group" style="float:left;width:20%;height:25px">
				<label class="control-label" style="width:100px">报关出口日期:</label>
				<div class="controls" style="margin-left:120px" >
					<input name="exportDate" type="text"  style="width:95%"  class="Wdate" value="<fmt:formatDate value="${psiTransportOrder.exportDate}" pattern="yyyy-MM-dd" />" />
				</div>
			</div>
		...
		<thead>
			<tr>
				   ...
				   <th style="width: 5%">出口金额(￥)</th>
				   <th style="width: 5%">进口金额</th>
		...
		<c:forEach items="${psiTransportOrder.items}"  var="item">
			<tr>
				...
				<td><input type="text" maxlength="11" style="width: 80%" name="lowerPrice" class="lowerPrice" value="${item.lowerPrice}" readonly/><input type="hidden" maxlength="11" style="width: 80%" name="productPrice" value="${item.productPrice}" class="productPrice" /></td>
				<td><input type="text" maxlength="11" style="width: 80%" name="importPrice" class="importPrice" value="${item.importPrice}" readonly/></td>
				<td><input type="text"  style="width: 80%" name="remark" readonly value="${item.remark}" ${psiTransportOrder.transportSta ne '0'?'readonly':'' }/></td>
			</tr>
		</c:forEach>
		</c:if>
