	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			...
			$("#add-row").on("click",function(e){
	            ...
	            tr.append("<td><input type='text' maxlength='11' style='width: 80%'  name='lowerPrice' class='lowerPrice'/><input type='hidden' maxlength='11' style='width: 80%' class='productPrice' name='productPrice' /></td>");
			...
			$("#inputForm").validate({
				...
				submitHandler: function(form){
					...
					$("#contentTable tbody tr").each(function(){
						var curkeyStr= $(this).find("input[name='productName']").val()+ $(this).find("select[name='countryCode']").val()+$(this).find("select[name='colorCode']").val()+$(this).find("select.sku").val()+","+$(this).find("select[name='offlineSta']").val();
					...
					if($("#transportType").val()!="3"){
						if(flag==2){
								top.$.jBox.tip("相同产品、国家、颜色、sku、相同线别,只能有一条记录!!!  "+twoStr, 'info',{timeout:3000});
								return false;
						}
					}
		...
		function  ajaxGetPrice(productId,countryCode,colorCode){
			...
			$.ajax({
				    ...
				    data: {
				    	...
				    	"colorCode":colorCode,
				    	"model":'${lcPsiTransportOrder.model}'
				    },
				    dataType: 'json',
				    success:function(data){ 
				    	console.log(data);
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/psi/lcPsiTransportOrder/">运单列表</a></li>
		<li class="active"><a href="#">${empty lcPsiTransportOrder.id ?'新建':'编辑'}
		<c:if test="${lcPsiTransportOrder.model eq '0'}">航空运单</c:if>
		<c:if test="${lcPsiTransportOrder.model eq '1'}">海运运单</c:if>
		<c:if test="${lcPsiTransportOrder.model eq '2'}">快递运单</c:if>
		<c:if test="${lcPsiTransportOrder.model eq '3'}">铁路运单</c:if>
		</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="lcPsiTransportOrder" action="${ctx}/psi/lcPsiTransportOrder/addSave" method="post" class="form-horizontal">
		<input type="hidden" name="transportNo" value="${lcPsiTransportOrder.transportNo}"/>
		<input type="hidden" name="model" value="${lcPsiTransportOrder.model}" id="model"/>
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
				...
				<div class="controls" style="margin-left:120px" >
					<select name="transportType" style="width: 100%" id="transportType">
						<option value="0" ${lcPsiTransportOrder.transportType eq '0'?'selected':''}>本地运输</option>	
						<option value="1" ${lcPsiTransportOrder.transportType eq '1'?'selected':''}>FBA运输</option>	
						<!-- <option value="2" ${lcPsiTransportOrder.transportType eq '2'?'selected':''}>批发运输</option> -->	
						<option value="3" ${lcPsiTransportOrder.transportType eq '3'?'selected':''}>线下运输</option>	
					</select>
				</div>
			</div>
		...
		<div style="float:left;width:98%">
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
				...
				<div class="controls" style="margin-left:120px" >
					<select name="fromStore.id" style="width: 100%" id="fromStoreId">
						<c:forEach items="${toStock}" var="stock">
						<c:if test="${stock.type eq '0' }">
							<option value="${stock.id}" ${stock.id eq '130' ? 'selected':'' }>${stock.stockSign}</option>
						</c:if>
						</c:forEach>		
					</select>
				</div>
			...
			<div class="control-group" style="float:left;width:25%;height:25px" id="toStoreDiv">
				...
				<div class="controls"  style="margin-left:120px" >
					<select name="toStore.id" style="width: 100%" id="toStore" >
						<c:forEach items="${toStock}" var="stock">
							<option value="${stock.id}" ${lcPsiTransportOrder.toStore.id eq stock.id ? 'selected':'' }>${fns:getDictLabel(stock.platform, 'platform', '')}&nbsp;${stock.stockSign}</option>
						</c:forEach>		
					</select>
				</div>
			</div>
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
					...
					起运<c:choose>
						<c:when test="${lcPsiTransportOrder.model eq '3'}">火车站</c:when>
						<c:when test="${lcPsiTransportOrder.model eq '1'}">港口</c:when>
					...
					<div class="controls" style="margin-left:120px" >
						<select name="orgin" style="width:95%">
							<c:forEach items="${fns:getDictList('transport_pod')}" var="dic">
								 <c:choose>
								 		<c:when test="${lcPsiTransportOrder.model eq '0' ||lcPsiTransportOrder.model eq '1' }">
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
					...
					<div class="controls" style="margin-left:120px" >
						<select name="destination" style="width:95%">
							 <c:forEach items="${fns:getDictList('transport_pod')}" var="dic">
								<option value="${dic.value}" ${lcPsiTransportOrder.destination eq dic.value ?'selected':''}>${dic.label}</option>
			             	 </c:forEach>
						</select>
					</div>
			</div>
		...
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:25%;height:25px">
					...
					<div class="controls" style="margin-left:120px" >
						<input name="boxNumber" type="text" readonly style="width:95%" value="${lcPsiTransportOrder.boxNumber}"/>
					</div>
			</div>
			...
			<div class="control-group" style="float:left;width:25%;height:25px">
				...
				<div class="controls" style="margin-left:120px" >
					<input name="pickUpDate"  type="text" style="width:95%" class="Wdate" required="required"  value="<fmt:formatDate value="${lcPsiTransportOrder.pickUpDate}" pattern="yyyy-MM-dd" />" />
				</div>
			</div>
		...
		<c:if test="${lcPsiTransportOrder.transportSta eq '0' }"> <div id="add-flag"><a href="#" id="add-row"><span class="icon-plus"></span>增加产品</a></div></c:if></div>
