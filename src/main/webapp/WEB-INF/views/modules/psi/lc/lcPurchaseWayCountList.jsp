	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			...
			$(".countryHref").click(function(){
				...
				$('#searchForm').attr('action','${ctx}/psi/lcPsiLadingBill/wayBillList?'+$.param(params));
			...
			$("#supplier").change(function(){
				$("#searchForm").attr("action","${ctx}/psi/lcPsiLadingBill/wayBillList?supplier.id="+$(this).val());
				$("#searchForm").submit();
			});
			...
			$("#country").change(function(){
				...
				$("#searchForm").attr("action","${ctx}/psi/lcPsiLadingBill/wayBillList?supplier.id="+supplierId+"&billNo="+$(this).val());
				$("#searchForm").submit();
			});
			...
			$("#uploadTypeFile").click(function(e){
				   ...
				   $.ajax({  
		                ...
		                success : function(data) { 
			                ...
			                var html="<div class='showChildrenHtml' style='text-align:center;margin-left:10px;' ><table style='width:98%;margin-top:10px'  class='table table-striped table-bordered table-condensed'><thead><tr><th  width='20%'>产品名称</th><th width='15%'>数量</th><th width='15%'>装箱体积(m³)</th><th width='15%'>毛重(kg)</th></tr></thead><tbody>";
			       			for(var i=0;i<data.length;i++){
			       				if(i==0){
			       				    html+="<tr><td><b>"+data[i][3]+"</b></td><td><b>"+data[i][2]+"</b></td><td><b>"+toDecimal(data[i][0])+"</b></td><td><b>"+toDecimal(data[i][1])+"</b></td></tr>";
			       				}else{
			       				    html+="<tr><td>"+data[i][3]+"</td><td>"+data[i][2]+"</td><td>"+toDecimal(data[i][0])+"</td><td>"+toDecimal(data[i][1])+"</td></tr>";
			       				}
			       			}
...
<body>
	...
	<form:form id="searchForm" modelAttribute="psiLadingBill" action="${ctx}/psi/lcPsiLadingBill/wayBillList" method="post" class="breadcrumb form-search" cssStyle="height: 30px;">
	...
	<label>预计收货日期：</label><input style="width: 100px" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',minDate:CurentTime(),isShowClear:true,onpicked:function(){$('#searchForm').submit();return true},oncleared:function(){$('#searchForm').submit();return true}});" readonly="readonly"  class="Wdate" type="text" name="createDate" value="<fmt:formatDate value="${psiLadingBill.createDate}" pattern="yyyy-MM-dd"/>" class="input-small" id="start" />
		...
		<a href="${ctx}/psi/lcPsiLadingBill/trial"> <input  class="btn btn-warning" type="button" value="全产品试算"/></a>
		&nbsp;&nbsp;<a href="#updateExcel" role="button" class="btn  btn-warning" data-toggle="modal" id="uploadFile">Excel文件试算</a> 
	</form:form>
		...
		<c:forEach items="${productMap}" var ="billEntry" varStatus="i">
			...
			<tr style="background-color:#D2E9FF;display: none" name="${productNameKey}"><td/><td>国家</td><td>颜色</td><td>订单号</td><td>未收货数量</td><td>po交期</td><td>预计交期</td><td/></tr>
				<c:forEach items="${orderMap[productNameKey]}" var="item">
					...
					<td><a href="${ctx}/psi/lcPurchaseOrder/view?id=${item.purchaseOrder.id}">${item.purchaseOrder.orderNo}</a></td><td>${item.quantityUnReceived}</td>
	...
	<div id="updateExcel" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true"> 
				 <form  enctype="multipart/form-data" id="uploadForm"  method="post" action='${ctx}/psi/lcPsiLadingBill/uploadFile'>
						  <div class="modal-header">
						    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
						    <h3 id="myModalLabel">产品文件上传</h3>
						  </div>
