	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			...
			$("#splitOrder").on("click",function(e){
				top.$.jBox.confirm("确认要拆分订单吗?","<spring:message code="sys_label_tips_msg"/>",function(v,h,f){
					if(v=="ok") {
						 $("#inputForm").attr("action","${ctx}/psi/lcPurchaseOrder/splitEditSave");
						 $("#inputForm").submit();
					}
				},{buttonsFocus:1});
			...
			$("input[name='quantityOrdered']").live("blur",function(){
				var tr = $(this).parent().parent();
				var supplierId = $("input[name='supplier.id']").val();
				var currencyType = $("select[name='currencyType']").children("option:selected").val();
				...
				tbody.find("input[name='quantityOrdered']").each(function(){
					var curProductId = $(this).parent().parent().find("select[name='product.id']").children("option:selected").val();
					var curColor = $(this).parent().parent().find("select[name='colorCode']").children("option:selected").val();
					if($(this).val()&&(curProductId==productId)&&(curColor==color)){
				...
				$("#contentTable tbody tr").each(function(){
					var curProductId = $(this).find("select[name='product.id']").children("option:selected").val();
					var curColor = $(this).find("select[name='colorCode']").children("option:selected").val();
					if((curProductId==productId)&&(curColor==color)){
						$(this).find("input[name='itemPrice']").val(price);
					}
		...
		$('#add-row').on('click', function(e){
            ...
            var tr = $("<tr></tr>");
            tr.append("<td> <input type='hidden' name='productName'/> <select style='width: 90%' class='productId'  name='product.id' > <c:forEach items='${purchaseOrder.supplier.products}' var='productSupplier' varStatus='i'><c:if test='${productSupplier.product.isSale ne 4 && productSupplier.product.reviewSta eq 1 }'><option value='${productSupplier.product.id}'>${productSupplier.product.name}</option></c:if></c:forEach></select> </td>");
            ...
            tr.append("<td> <input type='text' maxlength='11' style='width: 80%'  name='moldFee' class='price' /></td>");
            ...
            tr.append("<td><select name='remark' class='remark' style='width: 100%'><option value=''></option>"+options+"<option value='manutal'>手动填写</option></select><input name='remark'  type='text' style='margin-top:5px;display: none;'/></td>");
			...
			$("#btnSureSubmit").on('click',function(e){
				 if($("#inputForm").valid()){
					 var tips = '${purchaseOrder.supplier.payRemark}';
					 if(tips){
						 tips="供应商付款条款："+tips+"<br/>确认要申请审核？申请后将发送邮件通知审核人！";
					 }else{
						 tips="确认要申请审核？申请后将发送邮件通知审核人！";
					 }
					 top.$.jBox.confirm(tips,'系统提示',function(v,h,f){
							if(v=='ok'){
								$("#toReview").val("1");
								$("#inputForm").submit();
							}
		...
		function  ajaxGetPrice(supplierId,currency,productId,color,productQ){
			...
			$.ajax({
				    type: 'post',
				    async:false,
				    url: '${ctx}/psi/lcPurchaseOrder/ajaxPrice' ,
				    data: {
				    	"productId":productId,
...
<body>
	...
	<br/>
	<form:form id="inputForm" modelAttribute="purchaseOrder" action="${ctx}/psi/lcPurchaseOrder/editSave" method="post" class="form-horizontal">
		...
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<c:if test="${purchaseOrder.orderSta eq'0'}"><input id="splitOrder" class='btn btn-primary' type='button' value='拆分订单'/></c:if>
		</div>
		...
		<thead>
			<tr>
				   ...
				   <th style="width: 6%">单&nbsp;&nbsp;价&nbsp;&nbsp;</th>
				   <th style="width: 6%">
				   	<a href="#" style="color: #08c;" data-toggle="popover" data-html="true" rel="popover" data-content="单个产品分摊的模具费,返还时填负数">模具费</a>
				   </th>
				   <th style="width: 6%">总数量</th>
				   <th style="width: 6%">线下数</th>
				   <th style="width: 6%">拆单数</th>
		...
		<tbody>
			<c:forEach items="${purchaseOrder.items}" var="item" >
					<tr>
						...
						<input type="hidden" name="salesUser" value="${item.salesUser}" />
						...
						<select style="width: 90%" id="product" class='productId' name="product.id" >
							<c:forEach items="${purchaseOrder.supplier.products}" var="productSupplier" varStatus="i">
								<c:if test="${productSupplier.product.isSale ne '4' && productSupplier.product.reviewSta eq '1' }">
						...
						<td > <c:set value="${item.product.id}_${item.colorCode }" var="proColorKey" />
						<!-- ${not empty item.itemPrice?item.itemPrice:productPrices[proColorKey]} -->
						<input type="text" maxlength="11" style="width: 80%" name="itemPrice" class="price" value="${item.itemPrice}"/></td>
						<td><input type="text" maxlength="11" style="width: 80%" name="moldFee" class="price" value="${item.moldFee}"/></td>
						...
						<td > <input type="text" maxlength="11" style="width: 80%" name="chdQuantity" class="number" /></td>
