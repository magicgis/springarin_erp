				...
				if(ladingArrays.length>0){
					for(var i=0;i<ladingArrays.length;i++){
						...
						if(i==0){
				            ...
				            tr.append("<td><a href='${ctx}/psi/lcPurchaseOrder/view?id="+ladingDto.purchaseOrderId+"'</a>"+ladingDto.purchaseOrderNo+" </td>");
				            ...
				            tr.append("<td> <input readonly style='width: 90%' type='text' name='remark' /></td>");
				            ...
				            tr.append("<td><input type='hidden' name='id'/><input type='hidden' name='productConName'/><a href='${ctx}/psi/lcPurchaseOrder/view?id="+ladingDto.purchaseOrderId+"'</a>"+ladingDto.purchaseOrderNo+"</td>");
				            ...
				            tr.append("<td><input readonly style='width:90%' type='text' class='deposit' /></td>");
				            tr.append("<td><input readonly style='width:90%' type='text' class='payDepositAmount' /></td></shiro:hasPermission>");
				            tr.append("<td><input type='text' readonly value='"+pass+"'/></td>");
				            tr.append("<td> <input type='text' readonly style='width: 90%' class='partsTimely' /></td>");
				            tr.append("<td> <input type='text' readonly style='width: 90%' name='remark' /></td>");
				            tr.find("input[name='quantityLading']").val(ladingDto.quantityLading);
				            tr.find("input[name='quantityOffLading']").val(ladingDto.quantityOffLading);
				            tr.find("input[name='quantitySpares']").val(ladingDto.quantitySpares);
				            tr.find("input[name='quantitySure']").val(ladingDto.canSureQuantity);
...
<body>
	...
	<form:form id="inputForm" modelAttribute="psiLadingBill" action="${ctx}/psi/lcPsiLadingBill/sureSave" method="post" class="form-horizontal" enctype="multipart/form-data">
		...
		<div style="float:left;width:100%;">
			...
			<div class="control-group" style="float:left;width:30%" >
				<label class="control-label"><b>实际收货时间</b>:</label>
				<div class="controls" >
				<span>
					<input type="text" readonly value="${psiLadingBill.actualDeliveryDate}"/>
				</span>
				</div>
			</div>
			<div class="control-group" style="float:left;width:40%" >
				<label class="control-label"><b>应付款时间</b>:</label>
				<div class="controls" >
				<span>
					<input type="text" readonly value="<fmt:formatDate pattern='yyyy-MM-dd' value='${firstDay}'/>"/>
				</span>
				</div>
			</div>
		</div>
