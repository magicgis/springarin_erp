		...
		$(document).ready(function() {
			...
			$("#expByCountry").click(function(){
				top.$.jBox.confirm("<spring:message code="sys_label_tips_export"/>","<spring:message code="sys_label_tips_msg"/>",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/psi/lcPurchaseOrder/marketExport");
						$("#searchForm").submit();
						$("#searchForm").attr("action","${ctx}/psi/lcPurchaseOrder/");
					}
				},{buttonsFocus:1});
			...
			$("#expExcel").click(function(){
				...
				window.location.href = "${ctx}/psi/lcPurchaseOrder/exp?"+$.param(params);
				top.$.jBox.tip("<spring:message code='amazon_order_tips25'/> ！", 'loading',{timeout:10000});
			});
			
			$("#expPrice").click(function(){
				var params = {};
				params.createDate=$("input[name='createDate']").val();
				params.purchaseDate=$("input[name='purchaseDate']").val();
				params.orderNo=$("input[name='orderNo']").val();
				...
				params.isOverInventory=$("#isOverInventory").val(); 
				params['supplier.id']=$("#supplier").val();
				window.location.href = "${ctx}/psi/lcPurchaseOrder/exp2?"+$.param(params);
				top.$.jBox.tip("<spring:message code='amazon_order_tips25'/> ！", 'loading',{timeout:10000});
			});
			...
			$("#payExport").click(function(){
				...
				params.isOverInventory=$("#isOverInventory").val(); 
				params['supplier.id']=$("#supplier").val();
				window.location.href = "${ctx}/psi/lcPurchaseOrder/payExport?"+$.param(params);
				top.$.jBox.tip("<spring:message code='amazon_order_tips25'/> ！", 'loading',{timeout:10000});
			});
			...
			$("#expUnReceived").click(function(){
				...
				params.orderSta=$("#orderSta").val();
				params['supplier.id']=$("#supplier").val();
				window.location.href = "${ctx}/psi/lcPurchaseOrder/expUnReceived?"+$.param(params);
				top.$.jBox.tip("<spring:message code='amazon_order_tips25'/> ！", 'loading',{timeout:10000});
			});
			...
			$("#batchReceive").click(function(){
				...
				window.location.href = "${ctx}/psi/lcPsiLadingBill/batchReceive?"+$.param(params);
			});
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/psi/lcPurchaseOrder/">(理诚)采购订单列表</a></li>
	...
	<form:form id="searchForm" modelAttribute="lcPurchaseOrder" action="${ctx}/psi/lcPurchaseOrder/" method="post" class="breadcrumb form-search" cssStyle="height: 80px;">
		...
		<div style="height: 100px;line-height: 40px">
			...
			<form:select path="versionNo" style="width: 100px" id="country">
				...
				<c:forEach items="${fns:getDictList('platform')}" var="dic">
					<c:if test="${dic.value ne 'com.unitek'}">
						 <option value="${dic.value}" ${lcPurchaseOrder.versionNo eq dic.value ?'selected':''}  >${dic.label}</option>
					</c:if>      
				</c:forEach>	
			</form:select>&nbsp;&nbsp;
			<label>产品：</label>
			...
			<label>预收货：</label><input style="width: 100px" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',onpicked:function(){$('#searchForm').submit();return true}});" readonly="readonly"  class="Wdate" type="text" name="createDate" value="<fmt:formatDate value="${lcPurchaseOrder.createDate}" pattern="yyyy-MM-dd"/>" class="input-small" id="start"/>
			&nbsp;&nbsp;-&nbsp;&nbsp;<input style="width: 100px" onclick="WdatePicker({isShowClear:true,dateFmt:'yyyy-MM-dd',onpicked:function(){$('#searchForm').submit();return true},oncleared:function(){$('#searchForm').submit();return true}});" readonly="readonly"  class="Wdate" type="text" name="purchaseDate" value="<fmt:formatDate value="${lcPurchaseOrder.purchaseDate}" pattern="yyyy-MM-dd" />" id="end" class="input-small"/>
			...
			<form:select path="orderSta" style="width: 120px" id="orderSta">
				...
				<option value="7" ${lcPurchaseOrder.orderSta eq '7' ?'selected':''} >未完成</option>
			...
			<form:select path="isOverInventory" style="width: 120px" id="isOverInventory">
				...
				<option value="0" ${lcPurchaseOrder.isOverInventory eq '0' ?'selected':''} >普通</option>
				<option value="1" ${lcPurchaseOrder.isOverInventory eq '1' ?'selected':''} >特批</option>
			</form:select>
			...
			<div class="btn-group">
					...
					<ul class="dropdown-menu" >
						<li><a id="expExcel">导出excel</a></li>
						 <shiro:hasPermission name="psi:product:viewPrice">
						   <li><a id="expPrice">导出(价格)</a></li>
						</shiro:hasPermission>
						<li><a id="expByCountry">导出(销售)</a></li>
						<li><a id="expUnReceived">导出(未收货)</a></li>
						<shiro:hasPermission name="psi:order:financeReview">
				           <li><a id="payExport">导出(财务)</a></li>
			            </shiro:hasPermission> 
					</ul>
			</div>
	...
	<table id="contentTable" class="table table-bordered table-condensed">
		...
		<c:forEach items="${lcPurchaseOrder.tempOrders}" var="order" varStatus="j">
		<tr>
			...
			<c:choose>
				<c:when test="${j.index eq'0'}">
					<td>${order.id}</td>
					<td>
						<a href="${ctx}/psi/lcPurchaseOrder/view?id=${order.id}">${order.orderNo}</a>${lcPurchaseOrder.offlineSta eq '0'?'':'(含线下)'}
						<c:if test="${(lcPurchaseOrder.orderSta eq '0'||lcPurchaseOrder.orderSta eq '1'||lcPurchaseOrder.orderSta eq '2') &&order.needParts}">
							<br/>
							<c:if test="${lcPurchaseOrder.toPartsOrder eq '1'}">(<span style="font-weight: bolod;color: black">含配件已下单</span>)</c:if>
							<c:if test="${lcPurchaseOrder.toPartsOrder eq '0'}">(<span style="font-weight: bolod;color: red">含配件未下单</span>)</c:if>
						</c:if>
					</td>
				</c:when>
				<c:otherwise>
				...
				<c:when test="${j.index eq'0'}">
					...
					<c:if test="${order.isOverInventory ne '1'}">
						<c:if test="${order.orderSta eq '0'}"><span class="label label-important">草稿</span></c:if>
							<c:if test="${order.orderSta eq '1'}"><span class="label " style="background-color:#DCB5FF">已审核${not empty order.piReviewSta?'(待财务确认)':'(待确认)'}</span></c:if>
			...
			<c:if test="${j.index eq '0'}">
				...
				<c:if test="${order.orderSta ne '0'}">
					<c:if test="${fns:getUser().id == order.merchandiser.id}">
							<a class="btn btn-small" href="${ctx}/custom/sendEmail/form?sendEmail=${lcPurchaseOrder.supplier.id}&type=2&sendSubject=${order.orderNo}">发送PDF</a>
						</c:if>
				</c:if>
				 -->
					<shiro:hasPermission name="psi:order:edit">
					<c:if test="${order.orderSta ne '4' && order.orderSta ne '5' && order.orderSta ne '6'}">
						<a class="btn btn-small" href="${ctx}/psi/lcPurchaseOrder/edit?id=${order.id}">编辑</a>
					</c:if>
					<c:if test="${order.orderSta ne '4' && order.orderSta ne '5' && order.orderSta ne '6'}">
						<a class="btn btn-small" href="${ctx}/psi/lcPurchaseOrder/editDeliveryDate?id=${order.id}">编辑交期</a>
					</c:if>
					</shiro:hasPermission>
				<c:if test="${order.orderSta le '5' && order.orderSta ne '1' && order.orderSta ne '0' }">
					<shiro:hasPermission name="psi:upload:pi">
					<a class="btn btn-small"  href="${ctx}/psi/lcPurchaseOrder/uploadPi?id=${order.id}">上传PI</a>
					</shiro:hasPermission>
					<c:if test="${not empty order.piFilePath }">
						<a class="btn btn-small "  target="_blank" href="${ctx}/psi/lcPurchaseOrder/printPi?id=${order.id}">PI</a>
					</c:if>
				</c:if>
				<c:if test="${order.orderSta eq '1' }">
					<shiro:hasPermission name="psi:order:confirm">
						<a class="btn btn-small"  href="${ctx}/psi/lcPurchaseOrder/sure?id=${order.id}">${not empty order.piReviewSta?'编辑':''}确认</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="psi:order:edit">
						<a class="btn btn-small"  href="${ctx}/psi/lcPurchaseOrder/cancel?id=${order.id}" onclick="return confirmx('确认要取消该采购订单吗？', this.href)">取消</a>
					</shiro:hasPermission>
					<shiro:hasPermission name="psi:order:financeReview">
						<c:if test="${order.piReviewSta eq '0' }">
							<a class="btn btn-small"  href="${ctx}/psi/lcPurchaseOrder/sureReview?id=${order.id}">财务确认</a>
						</c:if>
					</shiro:hasPermission>
				</c:if>
				...
				<c:if test="${order.isOverInventory eq '1' }">
					<shiro:hasPermission name="psi:order:overReview">
						<a class="btn btn-small"  href="${ctx}/psi/lcPurchaseOrder/overReview?id=${order.id}">审核(超标)</a>
					</shiro:hasPermission>
				</c:if>
				
				<c:if test="${order.isOverInventory eq '0' }">
					<shiro:hasPermission name="psi:order:review">
						<a class="btn btn-small"  href="${ctx}/psi/lcPurchaseOrder/review?id=${order.id}">审核</a>
					</shiro:hasPermission>
				</c:if>
				...
				<shiro:hasPermission name="psi:order:edit">
					<a class="btn btn-small"  href="${ctx}/psi/lcPurchaseOrder/appRevise?id=${order.id}"
					onclick="return confirmx('${supplierAlertMap[order.supplier.id]}<br/>确认要申请审核订单吗?', this.href)">申请审核</a>
					<a class="btn btn-small"  href="${ctx}/psi/lcPurchaseOrder/cancel?id=${order.id}" onclick="return confirmx('确认要取消该采购订单吗？', this.href)">取消</a>
				</shiro:hasPermission>
				</c:if>
				...
				<c:if test="${order.orderSta eq '1'}">
				<shiro:hasPermission name="psi:order:review">
				<a class="btn btn-small"  href="${ctx}/psi/lcPurchaseOrder/toDraft?id=${order.id}" onclick="return confirmx('确认把该采购订单变成草稿吗？', this.href)" >变成草稿</a>
				</shiro:hasPermission>
				</c:if>
				
				<shiro:hasPermission name="psi:order:edit">
				<c:if test="${(order.orderSta eq '1' || order.orderSta eq '2') && order.toPartsOrder eq '0' && order.needParts}">
				<a class="btn btn-small" href="${ctx}/psi/lcPsiPartsOrderBasis/form?id=${order.id}">配件下单</a>
				</c:if>
				</shiro:hasPermission>
				
				<c:if test="${order.orderSta eq '2' ||order.orderSta eq '3' ||order.orderSta eq '4' ||order.orderSta eq '5'}">
				    <a class="btn btn-small"  target='_blank'  href="${ctx}/psi/lcPurchaseOrder/invoiceList?id=${order.id}">查看发票</a>
				</c:if>    
				    <%--  <shiro:hasPermission name="psi:order:edit">
				      <a class="btn btn-small"  href="${ctx}/psi/lcPurchaseOrder/genInvoice?id=${order.id}">生成发票</a>
				   </shiro:hasPermission>  --%>
			</c:if>
