	...
	(function() {
	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$('#add-row').on('click', function(e){
            ...
            tr.append("<td><select name='remark' class='remark' style='width:90%'><option value=''></option>"+options+"<option value='manutal'>手动填写</option></select><input name='remark'  type='text' style='margin-top:5px;display: none;'/></td>");
			...
			$("#inputForm").validate({
				...
				var totalQuantity=0;
					...
					if(totalQuantity!=$(".itemsQuantity").val()){
						top.$.jBox.tip("产品平台调整,balance总数必须相等！改前:"+$(".itemsQuantity").val()+",改后："+totalQuantity, 'info',{timeout:3000});
						return false;
					}
					if(flagNums==2){
