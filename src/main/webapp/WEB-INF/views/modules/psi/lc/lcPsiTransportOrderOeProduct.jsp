...
<head>
			...
			if(!(top)){
				...
				$("#inputForm").validate({
					...
					submitHandler: function(form){
						...
						if(flag==2){
							top.$.jBox.tip("相同sku,相同装箱数,相同线别,只能有一条记录!!!  "+twoStr, 'info',{timeout:3000});
							return false;
						}
	...
	<body>
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="#">(理诚)${empty psiTransportOrder.id ?'新建':'编辑'}${psiTransportOrder.model eq '1'?'海运':'铁路'}运单</a></li>
		</ul><br/>
		...
		<div style="float:left;width:98%">
			<div class="control-group" style="float:left;width:25%;height:25px">
					<label class="control-label" style="width:100px">${psiTransportOrder.model eq '1'?'船舶':'铁路'}公司:</label>
					<div class="controls" style="margin-left:120px" >
	...
	<blockquote  style="float:left;width:98%;height:25px">
		...
		<c:forEach items="${psiTransportOrder.items}"  var="item">
			<tr>
				...
				<td><input type="text" maxlength="11" style="width: 80%" name="packQuantity"  class="packQuantity" value="${item.packQuantity}"  ${not empty item.fbaInboundId?'readonly':'' }/></td>
