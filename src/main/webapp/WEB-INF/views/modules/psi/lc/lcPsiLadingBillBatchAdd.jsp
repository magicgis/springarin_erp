	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#supplier").on("change",function(e){
				...
				window.location.href = "${ctx}/psi/lcPsiLadingBill/add?"+$.param(params);
			});
				...
				if(ladingArrays.length>0){
					for(var i=0;i<ladingArrays.length;i++){
						...
						if(i==0){
				            ...
				            tr.append("<td> <input type='text' style='width: 80%' readonly class='packQuantity' /><input type='hidden' class='volume'/></td>");
							...
							tr.find(".volume").val(ladingDto.volume);
							...
							tr.append("<td><select class='skuS' style='width:98%' "+disable+">"+options+"</select><input type='hidden' name='sku'/></td>");
				            tr.append("<td><input type='hidden' name='id'/><input type='hidden' name='purchaseOrderItem.id'/><input type='hidden' class='balanceOffline'/><input type='hidden' name='itemPrice'/><input type='hidden' name='oldQuantityLading'/><input type='hidden' name='productConName'/><input type='hidden' name='itemPrice'/><input type='hidden' class='volume'/><input type='hidden' class='packQuantity'/><a target='_blank'  class='orderId purchaseOrderNo' href='#'/></td>");
				...
				if(aaa&&aaa.length>0){
					for(var i=0;i<aaa.length;i++){
						...
						if(i==0){
							...
							var tr =$("<tr class='notFirstRow'></tr>");
							tr.append("<td><select class='skuS' style='width:98%' "+disable+">"+options+"</select><input type='hidden' name='sku'/></td>");
				            tr.append("<td> <input type='hidden' name='productConName'/> <input type='hidden' class='balanceOffline'/><input type='hidden' name='purchaseOrderItem.id'/><input type='hidden' name='itemPrice'/> <input type='hidden' class='packQuantity'/><input type='hidden' class='volume'/> <a target='_blank'  class='orderId purchaseOrderNo' href='#'/></td>");
				            tr.append("<td> <input type='text' style='width: 80%' class='canLadingQuantity' readonly='readonly' /></td>");
				            tr.append("<td> <input type='text' style='width: 80%' name='quantityLading' /></td>");
				            tr.append("<td> <input type='text' style='width: 80%' name='quantityOffLading' /></td>");
			...
			$('#add-row').on('click', function(e){
	            ...
	            tr.append("<td> <input  style='width: 80%' type='text' class='packQuantity' readonly='readonly' /><input type='hidden' class='volume'/> </td>");
				...
				submitHandler: function(form){
					...
					$("#contentTable tbody tr").each(function(i,j){
						if($(this).find("input[name='quantityLading']").val()!=''&&$(this).find("input[name='quantityLading']").val()!=0){
							...
							var volume1 = $(this).find(".volume").val();
							if(packingQuantity!=''&&volume1!=''&&quantityLading!=''&&quantityOffLading!=''){
								var volume=((parseInt(quantityLading)+parseInt(quantityOffLading))/packingQuantity)*volume1;
					...
					if('${maxCap}'!=''&&'${timelyCap}'!=''){
						var maxVolume = parseFloat('${maxCap}');
						var curVolume = parseFloat('${timelyCap}');
						var rate =parseFloat((curVolume+parseFloat(totalVolume))*100/maxVolume).toFixed(2);
						if(rate>80){
							var tips="仓库最大库容："+maxVolume+",实时库容："+curVolume+",本次收货体积："+parseFloat(totalVolume).toFixed(2)+",库容占比达到："+rate+"%,请知悉!";
							top.$.jBox.tip(tips,"info",{timeout:5000});
						}
					}
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/psi/lcPsiLadingBill/form?id=${psiLadingBill.id}">新建收货单</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="psiLadingBill" action="${ctx}/psi/lcPsiLadingBill/addSave" method="post" class="form-horizontal">
	  ...
	  <input type='hidden' name="supplier.id" 		    value="${psiLadingBill.supplier.id}">
	    <blockquote>
			...
			<div class="control-group"  style="float:left;width:25%" >
				...
				<div class="controls" >
				<span>
					<input type="text" style="width:100px" name="deliveryDate" required="required" class="Wdate" value="<fmt:formatDate value="${psiLadingBill.deliveryDate}" pattern="yyyy-MM-dd" />" />
				</span>
				</div>
			</div>
