	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			...
			$("#btnNoSureSubmit").on('click',function(e){
				top.$.jBox.confirm('确认要跳过付款，直接进入已审核状态？','系统提示',function(v,h,f){
				if(v=='ok'){
					$("#paymentSta").val("3");
					$("#inputForm").submit();
				}
			...
			$("#supplier").on("change",function(e){
				...
				window.location.href = "${ctx}/psi/psiTransportPayment/add?"+$.param(params);
			});
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#">新建运单付款</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="psiTransportPayment" action="${ctx}/psi/psiTransportPayment/addSave" method="post" class="form-horizontal" enctype="multipart/form-data">
				...
				<span>
					<select style="width:95%" id="supplier" name="supplier.id">
						<c:forEach items="${suppliers}" var="supplier" varStatus="i">
							 <option value='${supplier.id}' ${psiTransportPayment.supplier.id eq supplier.id ?'selected':''}>${supplier.nikename}</option>;
						</c:forEach>
					</select>
				</span>
				</div>
	   ...
	   <div style="float: left"><blockquote><p style="font-size: 14px">付款项信息</p></blockquote></div><div style="float: left" id=errorsShow></div>
		...
		<c:forEach items="${unPaymentTransMap}" var="tranMap">
			...
			<tr>
				...
				<td><input style="width:90%" type="text" name="remark"/></td>
				...
				<c:forEach items="${tranMap.value}" var="payEntry" varStatus="i">
					<c:if test="${not empty payEntry[5]}">
						<c:forEach items="${fn:split(payEntry[5],',')}" var="attchment" varStatus="j">
							<a target="_blank" href="<c:url value='/data/site/psi/psiTransport/${attchment}'/>">${payEntry[1]}_${payEntry[2]}${j.index+1}</a>
							&nbsp;&nbsp;&nbsp;  
						</c:forEach>
					</c:if>
				</c:forEach>
