			...
			$("#inputForm").validate({
				submitHandler: function(form){
					...
					$("input[name='quantityOrdered']").each(function(e){
						...
						if(a!=0){
							twoStr=tr.find("select.productNameColor").children('option:selected').text();
							if(flagInt==1){
								flagInt=2;
							}
						};
					...
					if(flagInt==2){
						top.$.jBox.confirm("箱数不为整数，确定下单数量吗", 'tips', function (v, h, f) {
				            if (v == 'ok') {
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#">初级审核新品采购计划</a></li>
	</ul>
	<br/>
	
	<form:form id="inputForm" modelAttribute="purchasePlan" action="${ctx}/psi/purchasePlan/reviewSave" method="post" class="form-horizontal" enctype="multipart/form-data">
		...
		<thead>
			<tr>
				   ...
				   <th style="width: 5%">审批数量</th>
				   <th style="width: 15%">审批备注</th>
			...
			<c:if test="${not empty purchasePlan.id}">
				<c:forEach items="${purchasePlan.items}" var="item" >
					<tr>
						...
						<td>${item.remark}</td>
						<td>${item.quantity}</td>
						<td><input type="text" maxlength="11" style="width: 80%" name="quantityReview" class="number" value="${item.quantity}"/></td>
						<td><input type="text" style="width: 80%" name="remarkReview" /></td>
					</tr>
			</c:forEach>
		</c:if>
	</table>
