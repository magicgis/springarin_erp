	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			...
			$("#add-row").on("click",function(e){
	            ...
	            tr.append(" <td><input type='text' maxlength='11' style='width: 80%' name='productPrice' value='${item.productPrice}' class='price'/></td>");
...
<body>
	...
	<form:form id="inputForm" modelAttribute="psiTransportOrder" action="${ctx}/psi/psiTransportOrder/productEditSave" method="post" class="form-horizontal">
		...
		<c:if test="${not empty psiTransportOrder.localPath || not empty psiTransportOrder.tranPath || not empty psiTransportOrder.dapPath || not empty psiTransportOrder.otherPath || not empty psiTransportOrder.insurancePath || not empty psiTransportOrder.taxPath}">
		...
		<c:forEach items="${psiTransportOrder.items}"  var="item">
			<tr>
			   ...
			   <c:if test="${'1' eq canCreateFba &&(psiTransportOrder.transportSta eq '0' && psiTransportOrder.transportType eq '1')}">
				...
				<td><input type="text" maxlength="11" style="width: 80%" name="productPrice" value="${item.productPrice}" class="price"/></td>
