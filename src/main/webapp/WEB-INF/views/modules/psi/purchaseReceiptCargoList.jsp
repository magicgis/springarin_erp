	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			$("#lessCargo").click(function(e){
				...
				$("#searchForm").attr("action","${ctx}/psi/purchaseOrder/lessCargoList?supplier.id="+supplierId+"");
				$("#searchForm").submit();
			});
			...
			$("#supplier").change(function(){
				$("#searchForm").attr("action","${ctx}/psi/psiLadingBill/receiptCargoList?supplier.id="+$(this).val()+"");
				$("#searchForm").submit();
			});
...
<body>
	...
	<form:form id="searchForm" modelAttribute="psiLadingBill" action="${ctx}/psi/psiLadingBill/receiptCargoList" method="post" class="breadcrumb form-search" cssStyle="height: 30px;">
	<label>供应商：</label>
	...
	<table id="contentTable" class="table table-bordered table-condensed">
		...
		<c:forEach items="${productMap}" var ="billEntry" varStatus="i">
			...
			<c:forEach items="${billEntry.value.items}" var="item">
				...
				<td><a target="_blank" href="${ctx}/psi/psiLadingBill/view?id=${item.ladingBill.id}">${item.ladingBill.billNo }</a></td>	
