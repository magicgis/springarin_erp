	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			...
			$('#contentTable').on('change','select.remark',function(){
			  ...
			  $(".DeliveryDateEnter").editable({
				...
				success:function(response,newValue){
					...
					$.get("${ctx}/psi/purchaseOrder/updateSingelDeliveryDate?"+$.param(param),function(data){
						if(!(data)){    
							$this.text(oldVal);						
						}else{
			...
			$(".add-row").on('click', function(e){
	            ...
	            tr.append("<td><select class='remark' style='width:90%'><option value=''></option><option value='新品，第一次下单'>新品，第一次下单</option><option value='付款延后，推迟出货'>付款延后，推迟出货</option><option value='订单暂缓，推迟出货'>订单暂缓，推迟出货</option><option value='包材延误，推迟出货'>包材延误，推迟出货</option><option value='配件交期延误，推迟出货'>配件交期延误，推迟出货</option><option value='恰逢法定假期，推迟出货'>恰逢法定假期，推迟出货</option><option value='等待运输，推迟出货'>等待运输，推迟出货</option><option value='供应商出货延后，原材缺少'>供应商出货延后，原材缺少</option><option value='供应商出货延后，模具问题'>供应商出货延后，模具问题</option><option value='供应商出货延后，芯片问题'>供应商出货延后，芯片问题</option><option value='供应商出货延后，生产交期跟不上'>供应商出货延后，生产交期跟不上</option><option value='调节市场分配'>调节市场分配</option><option value='更新预计交期'>更新预计交期</option><option value='供应商备货，提前出货'>供应商备货，提前出货</option><option value='manutal'>手动填写</option></select><input name='remark' class='remarkInput' type='text' style='margin-top:5px;display: none;'/></td>");
			...
			$("#contentTable").on('click', '.save-row', function(e){
				...
				if(table.find(".deliveryTr")){
					...
					alert(orderUnReceivedQuantity);
					alert(unReceiveQuantity);
				...
				table.find(".deliveryTr").each(function(){
					...
					param.remark             = remark;
					$.get("${ctx}/psi/purchaseOrder/updateDeliveryDate?"+$.param(param),function(data){});
				});
						...
						var table=mainTr.find(".deliveryTable");
							...
							if(table.find("tr.deliveryTr").size()>0){
									table.find("tr.deliveryTr").each(function(){
										...
										if($(this).find(".quantityOff").val()<$(this).find(".quantityOffReceived").val()){
											bigSta=1;
											tips="线下数不能小于线下已收货数！";
											return false;
										}
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#">采购订单预计交期编辑</a></li>
	</ul>
	<br/>
	<form:form id="inputForm" modelAttribute="purchaseOrder" action="${ctx}/psi/purchaseOrder/deliveryDateSave" method="post" class="form-horizontal">
