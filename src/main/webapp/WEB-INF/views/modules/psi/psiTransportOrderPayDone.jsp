...
<head>
				...
				$("#btnSubmit").on("click",function(){
					top.$.jBox.confirm("您确定运单是已经付款完成了吗?,如果确定该运单将无法再进行付款","系统提示",function(v,h,f){
						if(v=="ok"){
							var params = {};
							params.id = $("#id").val();
							window.location.href = "${ctx}/psi/psiTransportOrder/payDoneSave?"+$.param(params);
						}
					},{buttonsFocus:1});
	...
	<body>
		<ul class="nav nav-tabs">
			...
			<li class="active"><a href="#">	<c:if test="${psiTransportOrder.model eq '0'}">航空</c:if><c:if test="${psiTransportOrder.model eq '1'}">海运</c:if><c:if test="${psiTransportOrder.model eq '2'}">快递</c:if>运单付款完成确认</a></li>
		</ul><br/>
		<form:form id="inputForm" modelAttribute="psiTransportOrder" action="${ctx}/psi/psiTransportOrder/payDoneSave" method="post" class="form-horizontal">
