...
<body>
	...
	<form:form id="inputForm" modelAttribute="psiTransportPayment" action="${ctx}/psi/psiTransportPayment/editSave" method="post" class="form-horizontal" enctype="multipart/form-data">
		...
		<div class="control-group" style="float:left;width:98%;">
			...
			<c:forEach items="${supplierFilePath}" var="fileMap">
					<c:forEach items="${fn:split(fileMap.value,',')}" var="attchment" varStatus="j">
						<a target="_blank" href="<c:url value='/data/site/psi/psiTransport/${attchment}'/>">${fileMap.key} ${j.index+1}</a>
						&nbsp;&nbsp;&nbsp;  
					</c:forEach>
			</c:forEach>
			</div>
