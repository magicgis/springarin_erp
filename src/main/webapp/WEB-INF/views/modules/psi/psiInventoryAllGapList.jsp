	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#contentTable").dataTable({"sDom": "<'row'<'spanexl'l><'spanexr'f>r>t<'row'<'spanexl'i><'spanexr'p>>",
			 ...
			 var cnt="<form  id='inputForm' action='${ctx}/psi/psiOutOfStockInfo/findAllGapInfo' method='post' > "+
			 ...
			 $("#expExcel").click(function(){
				 $("#expExcel").attr("href","${ctx}/psi/psiOutOfStockInfo/exportAllDetail?forecastType=0");
			});
				
			 $("#expExcel2").click(function(){
				 $("#expExcel2").attr("href","${ctx}/psi/psiOutOfStockInfo/exportAllDetail?forecastType=1");
			});
...
<body>
	...
	<ul class="nav nav-tabs">
		<c:forEach items="${fns:getDictList('platform')}" var="dic" varStatus="i">
			<c:if test="${dic.value ne 'com.unitek'}">
				<li><a href='${ctx}/psi/psiOutOfStockInfo/findGapInfo?country=${dic.value}' class="countryHref" href="#" key="${dic.value}">${dic.label}</a></li>
			</c:if>
		</c:forEach>	
		<li><a href="${ctx}/psi/psiOutOfStockInfo/findEuGapInfo">欧洲</a></li>
		<li  class="active"><a href="${ctx}/psi/psiOutOfStockInfo/findAllGapInfo">总计</a></li>
	</ul>
	...
	<table id="contentTable" class="table table-bordered table-condensed">
		...
		<c:forEach items="${gapMap}" var="gap"> 
			<c:forEach items="${gapMap[gap.key]}" var="gapCountry"> 
				   <c:forEach items="${gapMap[gap.key][gapCountry.key]}" var="gapType"> 
				    <c:forEach items="${gapMap[gap.key][gapCountry.key][gapType.key]}" var="nameType"> 
				       <tr> <c:set value='${gapType.key }_${gap.key}' var="name"/>
					        <c:set value='${gapType.key }_eu' var="euName"/>
				           <td style="text-align: center;vertical-align: middle;">
				             <span style="visibility:hidden;">${'0' eq gap.key?'0':('1' eq gap.key?'1':('2' eq gap.key?'3':'2')) }</span> 
				           ${'0' eq gapCountry.key?'销售预测':('1' eq gapCountry.key?'周日销':('2' eq gapCountry.key?'周日销(安)':'销售预测(安)')) }</td>
			               <td style="text-align: center;vertical-align: middle;"><a href="${ctx}/psi/psiInventory/productInfoDetail?productName=${gapType.key}" target="_blank">${gapType.key}</a>
			                   ...
			                   <c:if test="${'2' eq tranMap[gapType.key]['total']}"><span class="icon-plane"></span></c:if>
			               </td>
			                <td style="text-align: center;vertical-align: middle;">
				                    <c:if test="${'de' eq gap.key }">
				                        ${'0' eq powerMap[gapType.key]?'欧洲|EU':(fns:getDictLabel(gap.key,'platform','')) }
				                    </c:if>
				                    <c:if test="${'de' ne gap.key }">
				                       ${fns:getDictLabel(gap.key,'platform','')}
				                    </c:if>
				            </td>
			              
			                <td style="text-align: center;vertical-align: middle;">
				                  
				                   <c:if test="${'de' eq gapCountry.key }">
				                       <c:if test="${'0' eq powerMap[gap.key] }">
				                             ${fbaStock[euName].fulfillableQuantity }
				                       </c:if>
				                        <c:if test="${'0' ne powerMap[gap.key] }">
				                            ${fbaStock[name].fulfillableQuantity }
				                       </c:if>
				                   </c:if>
				                   <c:if test="${'de' ne gapCountry.key }">
				                       ${fbaStock[name].fulfillableQuantity }
				                   </c:if>
				            </td>
