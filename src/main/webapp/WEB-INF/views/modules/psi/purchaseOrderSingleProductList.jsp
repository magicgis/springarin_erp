...
<body>
	...
	<form:form id="searchForm" modelAttribute="purchaseOrder" action="${ctx}/psi/purchaseOrder/singleProduct" method="post" class="breadcrumb form-search" cssStyle="height: 40px;text-align:center;margin-top: 10px">
		...
		<div style="height: 40px;line-height: 40px;float:left">
			...
			<input name="snNo" type="text" maxlength="50" class="input-small" />
				...
				&nbsp;&nbsp;&nbsp;&nbsp;
				&nbsp;&nbsp;&nbsp;&nbsp;
			<a target="_blank" href="${ctx}/psi/psiInventory/productInfoDetail?productName=${productName}"><span style="font-size: 18px;font-weight: bold">${productName}</span></a>
		</div>
		...
		<tr>
			<c:choose>
				<c:when test="${j.index eq'0'}">
					<td>${order.id}</td>
					<td>
						<a href="${ctx}/psi/purchaseOrder/view?id=${order.id}">${order.orderNo}&nbsp;&nbsp;(WK<span style="color:red">${fns:getDateByPattern(purchaseOrder.createDate,"w")}</span>)&nbsp;[${order.snCode}]</a>
						<br/>
