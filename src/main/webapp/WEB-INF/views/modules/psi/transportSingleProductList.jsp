	...
	(function() {
	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			...
			$("#productName,#fromStoreId,#toCountry,#tranType,#tranModel,#startDate,#endDate").on("change",function(){
				$("#searchForm").submit();
			});
...
<body>
	...
	<form:form id="searchForm" modelAttribute="" action="${ctx}/psi/psiTransportOrder/singleTran" method="post" class="breadcrumb form-search" cssStyle="height: 40px;text-align:center;margin-top: 10px">
		<div style="height: 40px;line-height: 40px;float:left">
			<label>产品：</label>
			<select name="productName"  id="productName" style="width:150px">
				<c:forEach items="${productColors}" var="key">
						 <option value="${key}" ${key eq productName ?'selected':''}  >${key}</option>
				</c:forEach>	
			</select>&nbsp;&nbsp;&nbsp;&nbsp;
			<label>Model：</label>
			<select name="tranModel"  id="tranModel" style="width:60px">
			...
			<select name="tranType"  id="tranType" style="width:80px">
			 	 <option value="" >ALL</option>
			...
			<input id="go" class="btn btn-success" type="button"  value="去看理诚"/>
		</div>
	</form:form>
	<table id="contentTable" class="table table-bordered table-condensed">
		...
		<tbody>
			...
			<c:forEach items="${tranMap}" var="entryTran">
				<tr>
					<td>${entryTran.key}</td>
	...
	<table id="dataTable" class="table table-bordered table-condensed">
		<thead>
			<tr>
			<th width="10%">Month</th><th width="5%">Job No.</th><th width="5%">From</th><th width="5%">To</th>
			<th width="5%">Model</th><th width="5%">Transport Type</th><th>SKU</th><th width="5%">Quantity</th><th>KGS</th><th>CBM</th><th>Remark</th>
			</tr>
		</thead>
		<tbody>
		<c:forEach items="${list}" var="obj" varStatus="i">
			<tr>
				...
				<td><a target="_blank" href="${ctx}/psi/psiTransportOrder/view?transportNo=${obj[0]}">${obj[0]}</a></td>
				<td>
					<c:choose>
