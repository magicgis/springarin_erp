	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			...
			$("#supplier").on("change",function(e){
				...
				window.location.href = "${ctx}/psi/purchasePayment/add?"+$.param(params);
			});
			
			$("#btnExp").click(function(){
				window.location.href = "${ctx}/psi/purchasePayment/expPayment?supplierId="+$("#supplier").val();
			...
			$("#adjust-search").on('click',function(e){
				...
				$.ajax({
				    type: 'post',
				    async:false,
				    url: '${ctx}/psi/purchasePayment/ajaxOrderAjust' ,
				    data: {
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#">新建采购付款</a></li>
		<li ><a href="${ctx}/psi/purchaseOrder/moneyView">采购订单资金列表</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="purchasePayment" action="${ctx}/psi/purchasePayment/addSave" method="post" class="form-horizontal">
	   ...
	   <div align="right" style="font-size: 14px;margin: 5px 100px 5px 0px;">
			...
			&nbsp;&nbsp;&nbsp;&nbsp;
			<input class="btn btn-primary btnSureSubmit" type="button" value="申请" />&nbsp;&nbsp;&nbsp;&nbsp;
			...
			<tbody>
				<c:forEach items="${orders}" var="order">
					<tr>
						...
						<a target="_blank" href="${ctx}/psi/purchaseOrder/view?id=${order[1]}">${order[0]}</a></td>
				...
				<c:forEach items="${ladingItemDtos}" var="dto">
					<tr>
						...
						<a target="_blank" href="${ctx}/psi/psiLadingBill/view?id=${dto.ladingBillId}">${dto.billNo}</a></td>
						<td class="updateTips">逾${dto.delayDays}天${dto.rate}%档${fn:toUpperCase(dto.country eq 'com' ?'us':dto.country)}平台&nbsp;&nbsp;${dto.quantity}个&nbsp;&nbsp;单价 ${dto.itemPrice}&nbsp;&nbsp;定金比例 ${dto.deposit}%&nbsp;&nbsp;${dto.productName}&nbsp;&nbsp;已付${dto.totalPaymentAmount}</td>
