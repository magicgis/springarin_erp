	...
	(function() {
	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			eval('var orderMap=${partsOrderMap}');
			...
			$("#supplier").on("change",function(e){
				...
				window.location.href = "${ctx}/psi/psiPartsPayment/add?"+$.param(params);
			});
				...
				if(payType!=""){
					...
					if(payType=="0"){
						...
						if(order){
							...
							hrefStr="${ctx}/psi/psiPartsOrder/view?id="+order.id;
						};
					}else if(payType=="1"){
						...
						if(lading){
							...
							hrefStr="${ctx}/psi/psiPartsDelivery/view?id="+lading.id;
			...
			$("#add-row").on('click',function(e){
	            ...
	            tr.append("<td> <span id='masters'><input  type='hidden' name='unknowId'/><select name='billNo' style='width:80%' class='billNo' ></select></span><span class='updateTips' style='float:right;padding-right:2px'></span></td>");
			...
			$("#inputForm").validate({
				...
				submitHandler: function(form){
					...
					$("#contentTable tbody tr").each(function(){
						...
						if(currencyStr){
							if(currencyStr.indexOf($(this).find(".itemCurrencyType").val())<0){
								currencyFlag=2;
								return;
							}
						}else{
							currencyStr  = $(this).find(".itemCurrencyType").val();
						}
					});
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#">配件订单付款添加</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="psiPartsPayment" action="${ctx}/psi/psiPartsPayment/addSave" method="post" class="form-horizontal">
					...
					<span>
						<select id="supplier" name="supplier.id">
							<c:forEach items="${suppliers}" var="supplier" varStatus="i">
								 <option value='${supplier.id}' ${psiPartsPayment.supplier.id eq supplier.id ?'selected':''}>${supplier.nikename}</option>;
							</c:forEach>
						</select>
					</span>
					</div>
