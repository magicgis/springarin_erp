...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="#">配件订单付款列表</a></li>
		<li><a href="${ctx}/psi/psiPartsPayment/add">配件订单付款添加</a></li>
		<li><a href="${ctx}/psi/lcPsiPartsPayment">(理诚)配件订单付款列表</a></li>
		<li><a href="${ctx}/psi/lcPsiPartsPayment/add">(理诚)配件订单付款添加</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="psiPartsPayment" action="${ctx}/psi/psiPartsPayment/" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<div style="height: 100px;line-height: 40px">
			<div style="height: 40px;">
			<label>创建日期：</label><input style="width: 100px" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',onpicked:function(){$('#searchForm').submit();return true}});" readonly="readonly"  class="Wdate" type="text" name="createDate" value="<fmt:formatDate value="${psiPartsPayment.createDate}" pattern="yyyy-MM-dd"/>" class="input-small" id="start"/>
			&nbsp;&nbsp;-&nbsp;&nbsp;<input style="width: 100px" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',onpicked:function(){$('#searchForm').submit();return true}});" readonly="readonly"  class="Wdate" type="text" name="updateDate" value="<fmt:formatDate value="${psiPartsPayment.updateDate}" pattern="yyyy-MM-dd" />" id="end" class="input-small"/>
				&nbsp;&nbsp;&nbsp;&nbsp;
		...
		&nbsp;<input id="btnSubmit" class="btn btn-primary" type="submit" value="查询"/>
			...
			<select style="width:150px;" id="supplier" name="supplier.id">
				<option value="" ${psiPartsPayment.supplier eq '' ?'selected':''}><spring:message code="amazon_order_tips4"/></option>
	...
	<table id="contentTable" class="table table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="partsPayment">
			<tr id="${partsPayment.id}">
				...
				<td><a  href="${ctx}/psi/psiPartsPayment/view?id=${partsPayment.id}">${partsPayment.paymentNo}</a></td>
				<td class="paymentTotal">${partsPayment.amountTotal}</td>
				<td>
				...
				<c:if test="${partsPayment.paymentSta eq '0' }">
					<a class="btn btn-small"  href="${ctx}/psi/psiPartsPayment/edit?id=${partsPayment.id}">编辑</a>
					<a class="btn btn-small"  href="${ctx}/psi/psiPartsPayment/cancel?id=${partsPayment.id}&paymentSta=6" onclick="return confirmx('确认要取消该采购付款吗？', this.href)">取消</a>
				</c:if>
				<c:if test="${partsPayment.paymentSta eq '1' }">
					<a class="btn btn-small"  href="${ctx}/psi/psiPartsPayment/sure?id=${partsPayment.id}">确认</a>
					<div class="btn-group">
					   ...
					   <ul class="dropdown-menu" >
					      <li><a href="${ctx}/psi/psiPartsPayment/cancel?id=${partsPayment.id}&paymentSta=4" onclick="return confirmx('确认要取消该配件付款成草稿吗？', this.href)">取消成草稿</a></li>
					      <li><a href="${ctx}/psi/psiPartsPayment/cancel?id=${partsPayment.id}&paymentSta=5" onclick="return confirmx('确认要取消该配件付款吗？', this.href)">取消</a></li>
					   </ul>
					</div>
				</c:if>
