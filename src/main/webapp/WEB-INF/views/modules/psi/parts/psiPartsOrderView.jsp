				...
				<thead>
					<tr>
						...
						<c:forEach items="${itemEntry.value.items}" var="billItem" varStatus="i">
						 	  <tr>
						 	  <c:if test="${i.index==0}">
						 	  <td rowspan="${fn:length(itemEntry.value.items)}" style="text-align: center;vertical-align: middle;"><a href="${ctx}/psi/psiPartsDelivery/${itemEntry.value.billSta=='0'?'sure':'view'}?id=${billItem.partsDelivery.id}">${billItem.partsDelivery.billNo}</a></td>
						 	  </c:if>
			...
			<thead>
				...
				<tbody>
					<c:forEach items="${psiPartsOrder.paymentsMap}" var="pay" >
					 	  <tr>
					 	  <td><a href="${ctx}/psi/psiPartsPayment/view?id=${pay.value.id}">${pay.value.paymentNo}</a></td>   
							<td>
