			...
			$("#contentTable").on("change","select.partsId",function(e){
				...
				$("select.partsId").each(function(){
    				if($(this).val()!=partsId){
    					...
    					if('${psiPartsInventoryTaking.takingType}'=='0'){
		...
		$('#contentTable').on('click', '.remove-row', function(e){
			...
			$("#inputForm").validate({
				...
				submitHandler: function(form){
					var flag=0;
					var tips="";
					if('${psiPartsInventoryTaking.takingType}'=='1'){
...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/psi/psiPartsInventoryTaking/">配件库存盘点列表</a></li>
		<li class="active"><a href="#">配件库存盘点</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="psiPartsInventoryTaking" action="${ctx}/psi/psiPartsInventoryTaking/takingSave" method="post" class="form-horizontal">
		<input type="hidden" name="takingType"  value="${psiPartsInventoryTaking.takingType}"/>
		<div style="float:left;width:100% ;">
			<div class="control-group" style="float:left;width:50%">
				<label class="control-label">盘点类型:</label>
				<div class="controls">
					<input type="text" readonly="readonly" value="${psiPartsInventoryTaking.takingType eq '0' ? '盘入': '盘出'}"/>
				</div>
			</div>
			<div class="control-group" style="float:left;width:50%">
