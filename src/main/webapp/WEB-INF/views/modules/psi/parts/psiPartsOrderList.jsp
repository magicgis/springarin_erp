	  ...
	  s.parentNode.insertBefore(hm, s);
		...
		$(document).ready(function() {
			...
			$("#expExcel").click(function(){
				...
				window.location.href = "${ctx}/psi/psiPartsOrder/expOrder?"+$.param(params);
			...
			$("#batchReceive").click(function(){
				...
				window.location.href = "${ctx}/psi/psiPartsDelivery/batchReceive?"+$.param(params);
			});
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/psi/psiPartsOrder/">配件订单列表</a></li>
		<li><a href="${ctx}/psi/psiPartsOrder/add">新增配件订单</a></li>
		<li><a href="${ctx}/psi/lcPsiPartsOrder/">(理诚)配件订单列表</a></li>
		<li><a href="${ctx}/psi/lcPsiPartsOrder/add">(理诚)新增配件订单</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="psiPartsOrder" action="${ctx}/psi/psiPartsOrder/" method="post" class="breadcrumb form-search" cssStyle="height:80px">
	...
	<table id="contentTable" class="table  table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="psiPartsOrder">
			<tr>
				...
				<td><span class="partsOrderNo"><a href="${ctx}/psi/psiPartsOrder/view?id=${psiPartsOrder.id}">${psiPartsOrder.partsOrderNo}</a></span>&nbsp;${psiPartsOrder.sendEamil eq '1'?'<b>(Sent)</b>':''}</td>
				...
				<a class="btn btn-small" target="_blank" href="${ctx}/psi/psiPartsOrder/printPdf?id=${psiPartsOrder.id}" >PDF</a>&nbsp;&nbsp;
				<c:if test="${empty psiPartsOrder.purchaseOrderNo }">
					<c:if test="${psiPartsOrder.orderSta eq '0'}">
					<a class="btn btn-small"  href="${ctx}/psi/psiPartsOrder/edit?id=${psiPartsOrder.id}" >编辑</a>&nbsp;&nbsp;
					</c:if>
				</c:if>
				
				<c:if test="${psiPartsOrder.orderSta eq '0' }">
					<c:if test="${psiPartsOrder.sendEamil eq '0'}">
						<a class="btn btn-small"  href="${ctx}/psi/psiPartsOrder/sendEmail?id=${psiPartsOrder.id}"  onclick="return confirmx('确认要给该订单的供应商发邮件吗？', this.href)">Send</a>&nbsp;&nbsp;
					</c:if>
					<c:if test="${psiPartsOrder.sendEamil eq '1'}">
						<a class="btn btn-small"  href="${ctx}/psi/psiPartsOrder/sure?id=${psiPartsOrder.id}" >确认</a>&nbsp;&nbsp;
					</c:if>
				</c:if>
				
				<c:if test="${psiPartsOrder.orderSta eq '0' }">
					<a class="btn btn-small"  href="${ctx}/psi/psiPartsOrder/cancel?id=${psiPartsOrder.id}" onclick="return confirmx('确认要取消该配件订单吗？', this.href)">取消</a>&nbsp;&nbsp;
				</c:if>
