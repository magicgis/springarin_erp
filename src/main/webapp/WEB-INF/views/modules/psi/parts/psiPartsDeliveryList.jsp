	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#expExcel").click(function(){
				...
				window.location.href = "${ctx}/psi/psiPartsDelivery/exp?"+$.param(params);
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/psi/psiPartsDelivery/">配件收货列表</a></li>
		<li ><a href="${ctx}/psi/lcPsiPartsDelivery/">(理诚)配件收货列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="psiPartsDelivery" action="${ctx}/psi/psiPartsDelivery/" method="post" class="breadcrumb form-search" style="height:80px">
	...
	<table id="contentTable" class="table table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="psiPartsDelivery" varStatus="i">
			<tr>
				...
				<td><a href="${ctx}/psi/psiPartsDelivery/view?id=${psiPartsDelivery.id}">${psiPartsDelivery.billNo}</a></td>
								...
								<div class="btn-group">
								   ...
								   <button type="button" class="btn btn-small dropdown-toggle"  data-toggle="dropdown">
								    ...
								    <ul class="dropdown-menu" >
										<li><a  href="${ctx}/psi/psiPartsDelivery/sure?id=${psiPartsDelivery.id}">确认</a></li>
										<c:if test="${psiPartsDelivery.totalPaymentPreAmount+psiPartsDelivery.totalPaymentAmount ==0}">
											<li class="divider"></li>
											<li><a  href="${ctx}/psi/psiPartsDelivery/cancel?id=${psiPartsDelivery.id}" onclick="return confirmx('确认要取消该收货单吗？', this.href)">取消</a></li>
										</c:if>
									 </ul>
								</div>
							</c:if>
		   				<a target="_blank" class="btn btn-small"  href="${ctx}/psi/psiPartsDelivery/print?id=${psiPartsDelivery.id}">PDF</a>&nbsp;&nbsp;
		   				<c:if test="${empty psiPartsDelivery.attchmentPath}">
		   					<a class="btn btn-small"  href="${ctx}/psi/psiPartsDelivery/upload?id=${psiPartsDelivery.id}">上传凭证</a>&nbsp;&nbsp;
		   				</c:if>
				</td>
				</tr>
				<c:if test="${fn:length(psiPartsDelivery.items)>0}">
					...
					<c:forEach items="${psiPartsDelivery.items}" var="item">
						<tr style="background-color:#D2E9FF;display: none" name="${psiPartsDelivery.billNo}">
						<td></td><td><a target="_blank" href="${ctx}/psi/psiPartsOrder/view?id=${item.partsOrderItem.partsOrder.id}">
