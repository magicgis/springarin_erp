	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$("#takingInBtn").click(function(){
				var params = {};
				params.takingType='0';
				window.location.href = "${ctx}/psi/psiPartsInventoryTaking/form?"+$.param(params);
			});
			
			$("#inventoryAdjust").click(function(){
				window.location.href = "${ctx}/psi/psiPartsInventory/adjust";
			});
...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/psi/psiPartsInventory">配件库存列表</a></li>
		<li><a href="${ctx}/psi/lcPsiPartsInventory">(理诚)配件库存列表</a></li>
	</ul><br/>
	<form:form id="searchForm" modelAttribute="psiPartsInventory" action="${ctx}/psi/psiPartsInventory/" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
		<c:forEach items="${page.list}" var="psiPartsInventory">
			<tr>
				<td>${psiPartsInventory.partsName}</td>
				<td>${psiPartsInventory.poNotFrozen}</td>
				<td>${psiPartsInventory.poFrozen}</td>
				<td>${psiPartsInventory.stockNotFrozen}</td>
				<td>${psiPartsInventory.stockFrozen}</td>
				<td><a class="btn btn-small" href="${ctx}/psi/psiPartsInventoryLog?partsId=${psiPartsInventory.partsId}">查看日志</a></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
