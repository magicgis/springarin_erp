...
<body>
	...
	<div class="alert alert-info"><strong>预测产品为已有上架时间的新品和非淘汰品.</strong></div>
	<table id="contentTable" class="table table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="product" >
			<%-- <c:forEach items="${product.productNameWithColor}" var="name" > --%>
				    ...
				    <c:if test="${fn:contains(product[1],country) || 'eu' eq country ||'total' eq country}">
						<tr pro="${name}" class="${'total' eq country?'':'euro'}" style="${country eq 'eu'?'background-color:#D2E9FF;':''}">
							 <c:if test="${ii == 0}">
								...
								<td  style="text-align: center;vertical-align: middle" class="rowspan"><a target="_blank" href="${ctx}/psi/psiInventory/productInfoDetail?productName=${name}">${name}</a></td>
							 </c:if> 
									...
									<td  style="text-align: center;vertical-align: middle;">
										...
										<c:if test="${not empty realData && not empty forecast}">
											<c:choose>
												...
												<c:when test="${(realData gt forecast*1.2) || (realData lt forecast/1.2)}">
								...
								<td ${i.index==0?'class=blue':''} style="text-align: center;vertical-align: middle;">
									<c:if test="${realData==0}">
										<c:if test="${'eu' ne country&&'total' ne country}">
											<a href="#" style="${fns:getUser().id ==data[name][country][month].lastUpdateBy.id?'color: fuchsia;font-weight: bold;':''}"  data-type="text" data-pk="1"  data-title="update" class="editable editable-click editor" data-original-title="" title="">${forecast}</a>
									...
									<c:if test="${realData >0 && not empty forecast }">
										<c:if test="${'eu' ne country&&'total' ne country}">
											<a href="#" style="${fns:getUser().id ==data[name][country][month].lastUpdateBy.id?'color: fuchsia;font-weight: bold;':''}"  data-type="text" data-pk="1"  data-title="实时销量:${realData}" class="editable editable-click editor" data-original-title="" title="">${forecast}</a>
											<input class="forecastId" type="hidden" value="${data[name][country][month].id}"/>
											<input class="productName" type="hidden" value="${name}"/>
											<input class="productId" type="hidden" value="${product[2]}"/>
											<input class="month" type="hidden" value="${month}"/>
										...
										<c:if test="${'eu' ne country&&'total' eq country}">
											<a href="#" style="${fns:getUser().id ==data[name][country][month].lastUpdateBy.id?'color: fuchsia;font-weight: bold;':''}"  data-type="text" data-pk="1"  data-title="实时销量:${realData}"  data-original-title="" title="">${forecast}</a>
									...
									<c:if test="${i.index==0 && not empty forecast && saleData[name][country][dates[2]] >0 }">
										<c:if test="${(forecast/saleData[name][country][dates[2]]) >1.15 }">
											<br/><span style="color: red;font-weight: bold;">预测增幅  ${fns:roundUp(((forecast/saleData[name][country][dates[2]])-1)*100)}%</span>											
										</c:if>									
									</c:if>
