...
<%@ include file="/WEB-INF/views/include/taglib.jsp"%>
	<div style="width: 550px;border:1px solid #ccc">
		<div id="${country}${productName}${start}${end}" style="height:260px"></div>
	</div>
	<script type="text/javascript">
	    ...
	    require(
	        ...
	        function(ec) {
	            myChart = ec.init(document.getElementById('${country}${productName}${start}${end}'));
	            myChart.showLoading({
				...
				myChart.hideLoading();
	            var option = {
	            	title:{text:"${productName}排名统计图",x:'center'},		
	                tooltip : {
	                    trigger: 'item'
	                },
	                legend: {
	                ...
	                series : [
						{
						      name:'${productName}',
						      type:'line',
						      data:[<c:forEach items="${xAxis}" var="x" varStatus="i">${empty rankMap[x]?0:rankMap[x]}${i.last?'':','}</c:forEach>]
						  }
	                ]
	            };
