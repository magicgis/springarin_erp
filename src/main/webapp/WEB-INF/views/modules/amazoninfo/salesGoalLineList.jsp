	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			arr.each(function() {
				...
				console.log(parseInt($(this).find("td :eq(9)").text().split(',').join('')));
			...
			var cnt="<form  id='inputForm' action='${ctx}/amazoninfo/enterpriseGoal/findMonthLineGoal' method='post' >"+
			"&nbsp;&nbsp;&nbsp;&nbsp;时间: "+
			...
			"</form> ";
			 ...
			 $("#export").click(function(){
				 $("#inputForm").attr("action","${ctx}/amazoninfo/enterpriseGoal/exportLineTotalData");
				 $("#inputForm").submit();
				 $("#inputForm").attr("action","${ctx}/amazoninfo/enterpriseGoal/findMonthLineGoal");
			 });
...
<body>
	...
	<!-- </div>  -->
	<ul class="nav nav-tabs">
		<li class="${empty enterpriseGoal.country?'active':''}"><a href="${ctx}/amazoninfo/enterpriseGoal/findMonthLineGoal" key="">总计</a></li>
		<li class="${enterpriseGoal.country eq 'notUs'?'active':''}"><a href="${ctx}/amazoninfo/enterpriseGoal/findMonthLineGoal?country=notUs">总计(不含美国)</a></li>
		<li class="${enterpriseGoal.country eq 'en'?'active':''}"><a href="${ctx}/amazoninfo/enterpriseGoal/findMonthLineGoal?country=en">英语国家</a></li>	
		<li class="${enterpriseGoal.country eq 'notEn'?'active':''}"><a href="${ctx}/amazoninfo/enterpriseGoal/findMonthLineGoal?country=notEn">非英语国家</a></li>	
		...
		<thead>
			<tr>
			   <!--  <th style="text-align: center;vertical-align: middle;width:2%;">序号</th> -->
				...
				<th style="text-align: center;vertical-align: middle;width:10%; color:#08c">总销售额(€)</th>
				<c:forEach items="${allLine}" var="dic">
			   		<th style="text-align: center;vertical-align: middle;width:12%;">${dic.value} 产品线(€)</th>
			   		<th style="text-align: center;vertical-align: middle;width:12%; color:#08c">销售额(€)</th>
				</c:forEach>
			</tr>
		</thead>
		<tbody>
			 <c:forEach items="${map}" var="enterprise" varStatus="i"> 
				<tr>
					...
					<c:forEach items="${allLine}" var="dic">
					      <c:set var="totalGoal" value="${totalGoal+map[enterprise.key][dic.key]}" /> 
					      <c:set var="totalSale" value="${totalSale+saleData['total'][dic.value][enterprise.key].sales}" /> 
					</c:forEach>
					<td style="text-align: center;vertical-align: middle;">
					   <c:if test="${totalGoal==0 }">
					   	<fmt:formatNumber pattern="#,##0" value="${map[enterprise.key]['total'] }"  maxFractionDigits="0" />
					   </c:if>
					   <c:if test="${totalGoal>0 }">
					...
					<c:forEach items="${allLine}" var="dic">
					      <%--<c:set var="totalGoal" value="${totalGoal+map[enterprise.key][dic.key]}" />  --%>
						  <td style="text-align: center;vertical-align: middle;">
				   			<fmt:formatNumber pattern="#,##0" value="${map[enterprise.key][dic.key] }"  maxFractionDigits="0" />
						 </td>
					      <%--<c:set var="totalSale" value="${totalSale+saleData['total'][dic.value][enterprise.key].sales}" />  --%>
				   			<td style="text-align: center;vertical-align: middle;width:12%; color:#08c">
				   				<fmt:formatNumber pattern="#,##0" value="${saleData['total'][dic.value][enterprise.key].sales }"  maxFractionDigits="0" />
				   			</td>
					</c:forEach>
				</tr>
			</c:forEach>
