...
<html>
<head>
	<title>amazonFacebookList</title>
	...
	<script type="text/javascript">
		...
		function exportList(){
			$("#searchForm").attr("action","${ctx}/amazoninfo/amazonAndFacebook/exportAmazonFacebookList");
			$("#searchForm").submit();
			$("#searchForm").attr("action","${ctx}/amazoninfo/amazonAndFacebook/amazonFacebookList");
		}
		function deleteDate(){
			...
			top.$.jBox.confirm("确定删除这些数据？","<spring:message code="sys_label_tips_msg"/>",function(v,h,f){
				if(v=="ok"){
					   ...
					   $("#searchForm").attr("action","${ctx}/amazoninfo/amazonAndFacebook/deleteAmazonFacebookArr?ids="+idsAll);
					   $("#searchForm").submit();
					   $("#searchForm").attr("action","${ctx}/amazoninfo/amazonAndFacebook/amazonFacebookList");
				}
			},{buttonsFocus:1});
		...
		function updateProfit(){
			   $("#searchForm").attr("action","${ctx}/amazoninfo/amazonAndFacebook/updateProfit");
			   $("#searchForm").submit();
			   $("#searchForm").attr("action","${ctx}/amazoninfo/amazonAndFacebook/amazonFacebookList");
		}
	</script>
...
<body>
	<ul class="nav nav-tabs">
		...
		<li  class="active"><a href="${ctx}/amazoninfo/amazonAndFacebook/amazonFacebookList">Amazon订单数据</a></li>
		<li><a href="${ctx}/amazoninfo/amazonAndFacebook/amazonFacebookRelationship">关联数据</a></li>
	</ul>
	<div style="display: none" id="searchContent">
		<form:form id="searchForm" modelAttribute="amazonFacebookReport" action="${ctx}/amazoninfo/amazonAndFacebook/amazonFacebookList" method="post">
		&nbsp;&nbsp;
		<label><b>Date Shipped：</b></label><input style="width: 100px" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" readonly="readonly"  class="Wdate" type="text" name="dateShipped" value="<fmt:formatDate value="${amazonFacebookReport.dateShipped}" pattern="yyyy-MM-dd"/>" class="input-small" id="start"/>
				&nbsp;至&nbsp;<input style="width: 100px" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" readonly="readonly"  class="Wdate" type="text" name="endDate" value="<fmt:formatDate value="${amazonFacebookReport.endDate}" pattern="yyyy-MM-dd" />" id="end" class="input-small"/>
	...
	<!-- 	<input id="updateBtn" class="btn btn-primary" type="button" value="更新" onclick='updateProfit();'/>	&nbsp;&nbsp; -->
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th style="width: 3%"><input type="checkbox" id="checkall" checked></th>
		<th>Market</th>
					<th>Product Line</th>
					<th>Items Name</th>
					<th>ASIN</th>
		...
		<c:forEach items="${page.list}" var="report">
			<tr>
				...
				<td><a title='${report.itemName}'>${fns:abbr(report.itemName,20)}</a></td>
				<td>${report.asin}</td>
				<td>${report.seller}</td>
				<td>${report.trackingId}</td>
				<td>${report.dateShipped}</td>
				<td>${report.price}</td>
				<td>${report.advertisingFeeRate}</td>
				<td>${report.itemsShipped}</td>
				<td>${report.revenue}</td>
				<td>${report.advertisingFees}</td>
				<td>${report.deviceType}</td>
				<td><a class="btn btn-danger" href="${ctx}/amazoninfo/amazonAndFacebook/deleteAmazonFacebook?id=${report.id}">delete</a></td>
			</tr>
		</c:forEach>
		</tbody>
	</table>
