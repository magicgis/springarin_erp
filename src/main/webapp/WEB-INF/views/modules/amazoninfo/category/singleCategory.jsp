...
<html>
<head>
<title>目录:${categoryName} 分析</title>
...
<body>
    <div class="alert">
	  	<strong>平台:${country} 账号:${name}  目录:${categoryName} 分析</strong>&nbsp;&nbsp;&nbsp;&nbsp;
	</div>
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
			...
			<tbody>
				<c:forEach items="${data}" var="row" varStatus="i">
					<tr>
						<td><a target="_blank" href="https://www.amazon.${country eq 'US'?'com':'de'}/dp/${row[1]}">${fns:abbr(row[0],60)}</a></td>
						<td>${row[2]}</td>
						<td>${row[3]}</td>
						<td><fmt:formatNumber value="${row[4]}" pattern="#.##" minFractionDigits="2" /></td>
						<td><fmt:formatNumber value="${row[5]}" type="number"  /></td>
						<td>${row[6]}</td>
						<td><a class="btn btn-small btn-success"  href="${ctx}/amazoninfo/productCatalog/product?country=${country}&name=${name}&merchantCustomerId=${merchantCustomerId}&categoryName=${categoryName}&asin=${row[1]}&asinName=${fns:abbr(row[0],10)}" target="_blank">产品分析</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
