...
<body>
	...
	<form:form id="inputForm" modelAttribute="menu" action="${ctx}/sys/menu/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<select name="remarks" multiple class="multiSelect required"  style="width:40%" id="remarks">
					<c:forEach items="${fn:split(menu.remarks, ',')}" var="remark">
						<option value="${remark}" selected>${fns:getDictLabel(remark,'server_flag','')}</option>	
					</c:forEach>
					<c:forEach items="${serverFlagMap}" var="dic">
