...
<head>
	...
	<script type="text/javascript">
		...
		function changeJobStatus(action, jobName, jobGroup){
			...
			$("#searchForm").attr("action","${ctx}/sys/job/runWsTask");
			$("#searchForm").submit();
		}
	</script>
</head>
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/sys/job/">本地任务</a></li>
		<c:forEach items="${wsConfigs}" var="config">
			<li class="${name eq config.accountName?'active':''}"><a href="${ctx}/sys/job/wsList?name=${config.accountName}&host=${config.serverIp}">${config.accountName}(${config.serverIp})</a></li>
		</c:forEach>
	</ul>
